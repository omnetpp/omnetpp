...
void cMySQLNetBuilder::initialize()
{
    ...
    cModule *mod = (!modName || !modName[0]) ? getParentModule() : simulation.getModuleByPath(modName);
...
cGate *cMySQLNetBuilder::getFirstUnusedSubmodGate(cModule *modp, const char *gatename)
{
    ...
    if (baseId<0)
        throw cRuntimeError("Module %s has no %s[] gate",modp->getFullPath().c_str(), gatename);
...
void cMySQLNetBuilder::readAndBuild(MYSQL *mysql, const char *networkName, cModule *parentmod)
{
    ...
    while ((row = mysql_fetch_row(res))!=NULL)
    {
        ...
        // set display string
        mod->getDisplayString().parse(dispstr);
    }
