...
const char *NEDParser::getBuiltInDeclarations()
{
    return
        ...
        "    @class(cDelayChannel);\n"
        "    bool disabled = false;\n"
        "    double delay = 0s @unit(s); // propagation delay\n"
        "}\n"
        "\n"
        ...
        "    bool disabled = false;\n"
        "    double delay = 0s @unit(s); // propagation delay\n"
        "    double datarate = 0bps @unit(bps); // bits per second\n"
        "    double ber = 0; // bit error rate (BER)\n"
        "    double per = 0; // packet error rate (PER)\n"
        "}\n"
        "\n"
