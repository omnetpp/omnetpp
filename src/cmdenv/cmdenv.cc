...
Register_PerRunConfigOption(CFGID_GLOBAL_LOGLEVEL, "cmdenv-global-log-level", CFG_STRING, "DEBUG", "Specifies the global level of detail recorded by log statements, output below the specified level is omitted. This setting is with AND relationship with per-component log level settings. Available values are (case insensitive): off, fatal, error, warn, info, detail, debug or trace. Note that the level of detail is also controlled by the specified per component log levels and the COMPILETIME_LOGLEVEL macro that is used to completely remove log statements from the executable.");
...
void Cmdenv::configure(cComponent *component)
{
    ...
    LogLevel level = cLogLevel::getLevel(getEnvir()->getConfig()->getAsString(component->getFullPath().c_str(), CFGID_CMDENV_LOGLEVEL).c_str());
