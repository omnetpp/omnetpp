 ...
 * exponent, which is global (shared by all SimulTime instances), and falls in
 ...
 * and via template functions for integer types. Performance note:
...
class COMMON_API SimulTime
{
  ...
  public:
     ...
     */
    SimulTime() {t=0;}
    template<typename T> SimulTime(T d) {operator=(d);}

    /** @name Arithmetic operations */
    //@{
    const SimulTime& operator=(double d) {t = (int64)(fscale*d); return *this;}
    const SimulTime& operator=(const SimulTime& x) {t=x.t; return *this;}
    template<typename T> const SimulTime& operator=(T d) {t = (int64)(dscale*d); return *this;}

    const SimulTime& operator+=(const SimulTime& x) {t+=x.t; return *this;}
    const SimulTime& operator-=(const SimulTime& x) {t-=x.t; return *this;}

    const SimulTime& operator*=(double d) {t=(int64)(t*d); return *this;} //XXX to be checked on Linux, see below
    const SimulTime& operator/=(double d) {t=(int64)(t/d); return *this;} //XXX to be checked on Linux, see below
    template<typename T> const SimulTime& operator*=(T d) {t*=d; return *this;}
    template<typename T> const SimulTime& operator/=(T d) {t/=d; return *this;}

    bool operator==(const SimulTime& x) const  {return t==x.t;}
    bool operator!=(const SimulTime& x) const  {return t!=x.t;}
    bool operator< (const SimulTime& x) const  {return t<x.t;}
    bool operator> (const SimulTime& x) const  {return t>x.t;}
    bool operator<=(const SimulTime& x) const  {return t<=x.t;}
    ...
    friend const SimulTime operator+(const SimulTime& x, const SimulTime& y);
    friend const SimulTime operator-(const SimulTime& x, const SimulTime& y);

    friend const SimulTime operator*(const SimulTime& x, double d);
    friend const SimulTime operator*(double d, const SimulTime& x);
    friend const SimulTime operator/(const SimulTime& x, double d);
    friend double operator/(const SimulTime& x, const SimulTime& y);

    /*XXX todo
    template<typename T> friend const SimulTime operator*(const SimulTime& x, T d);
    template<typename T> friend const SimulTime operator*(T d, const SimulTime& x);
    template<typename T> friend const SimulTime operator/(const SimulTime& x, T d);
    */
    //@}
     ...
     */
    char *str(char *buf) {char *endp; return SimulTime::ttoa(buf, t, scaleExp(), endp);}
     ...
     */
    const SimulTime& setRaw(int64 l) {t = l; return *this;}

    /**
     * Returns the largest simulation time that can be represented using the
     * present scale exponent.
     */
    static const SimulTime maxTime() {return SimulTime().setRaw((((int64)1) << 63) - 1);}
     ...
     */
    static const SimulTime parse(const char *s);
     ...
     */
    static const SimulTime parse(const char *s, const char *&endp);
...
inline const SimulTime operator+(const SimulTime& x, const SimulTime& y)
{
    return SimulTime(x)+=y;
}
...
inline const SimulTime operator*(const SimulTime& x, double d)
{
    return SimulTime(x)*=d;
}

inline const SimulTime operator*(double d, const SimulTime& x)
{
    return SimulTime(x)*=d;
}
...
inline double operator/(const SimulTime& x, const SimulTime& y)
{
    return (double)x.t / (double)y.t;
}

inline std::ostream& operator<<(std::ostream& os, const SimulTime& x)
{
    char buf[64]; char *endp;
    return os << SimulTime::ttoa(buf, x.raw(), SimulTime::scaleExp(), endp);
}

/**
 * simtime_t version of floor(double) from math.h.
 */
inline const SimulTime floor(const SimulTime& x)
{
    int64 u = SimulTime::scale();
    int64 t = x.raw();
    return SimulTime().setRaw(t - t % u); //XXX test: also OK for negative t?
}
 ...
 */
inline const SimulTime floor(const SimulTime& x, const SimulTime& unit, const SimulTime& offset = SimulTime())
{
    int64 off = offset.raw();
    int64 u = unit.raw();
    int64 t = x.raw() - off;
    return SimulTime().setRaw(t - t % u + off); //XXX test: also OK for negative t?
}

/**
 * simtime_t version of ceil(double) from math.h.
 */
inline const SimulTime ceil(const SimulTime& x)
{
    int64 u = SimulTime::scale();
    int64 t = x.raw() + u-1;
    return SimulTime().setRaw(t - t % u); //XXX test: also OK for negative t?
 ...
 */
inline const SimulTime ceil(const SimulTime& x, const SimulTime& unit, const SimulTime& offset = SimulTime())
{
    int64 off = offset.raw();
    int64 u = unit.raw();
 ...
 */
inline const SimulTime fabs(const SimulTime& x)
{
    return x.raw()<0 ? SimulTime().setRaw(-x.raw()) : x;
}

/**
 * simtime_t version of fmod(double,double) from math.h.
 */
inline const SimulTime fmod(const SimulTime& x, const SimulTime& y)
{
    return SimulTime().setRaw(x.raw() % y.raw()); //XXX test: also OK for negative x or y?
}

/**
 * Returns the greater of the two arguments.
 */
inline const SimulTime max(const SimulTime& x, const SimulTime& y)
{
    return x > y ? x : y;
}
 ...
 */
inline const SimulTime min(const SimulTime& x, const SimulTime& y)
{
    return x < y ? x : y;
}
