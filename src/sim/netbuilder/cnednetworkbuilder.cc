...
cGate *cNEDNetworkBuilder::getFirstUnusedParentModGate(cModule *mod, const char *gatename)
{
    // same code as generated by nedtool into _n.cc files
    int baseId = mod->findGate(gatename);
    if (baseId<0)
        throw new cException("dynamic module builder: %s has no %s[] gate",mod->fullPath(), gatename);
    int n = mod->gate(baseId)->size();
    for (int i=0; i<n; i++)
        if (!mod->gate(baseId+i)->isConnectedInside())
            return mod->gate(baseId+i);
    throw new cException("%s[] gates are all connected, no gate left for `++' operator",mod->fullPath(), gatename);
}

cGate *cNEDNetworkBuilder::getFirstUnusedSubmodGate(cModule *mod, const char *gatename)
{
    // same code as generated by nedtool into _n.cc files
    int baseId = mod->findGate(gatename);
    if (baseId<0)
    ...
    for (int i=0; i<n; i++)
        if (!mod->gate(baseId+i)->isConnectedOutside())
            return mod->gate(baseId+i);
    const int delta = 1;
    mod->setGateSize(gatename,n+delta);
    return mod->gate(baseId+n);
}
                                       ...
                                       bool isplusplus)
{
    if (isplusplus && gateindexp)
        throw new cException("dynamic module builder: \"++\" and gate index expression cannot exist together");
