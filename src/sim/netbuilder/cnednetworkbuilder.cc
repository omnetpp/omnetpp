...
cGate *cNEDNetworkBuilder::getFirstUnusedParentModGate(cModule *modp, const char *gatename)
{
    ...
    if (baseId<0)
        throw new cException("dynamic module builder: %s has no %s[] gate",modp->fullPath(), gatename);
    int n = modp->gate(baseId)->size();
    for (int i=0; i<n; i++)
        if (!modp->gate(baseId+i)->isConnectedInside())
            return modp->gate(baseId+i);
    throw new cException("%s[] gates are all connected, no gate left for `++' operator",modp->fullPath(), gatename);
}
