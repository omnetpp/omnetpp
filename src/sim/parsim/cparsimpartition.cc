...
Register_GlobalConfigOption(CFGID_PARSIM_DEBUG, "parsim-debug", CFG_BOOL, "false", "With `parallel-simulation=true`: turns on printing of log messages from the parallel simulation code.");
...
Register_GlobalConfigOption(CFGID_PARSIM_PROCESS_PARTITIONID, "parsim-process-partitionid", CFG_INT, nullptr, "If `parallel-simulation=true` and `parsim-mode=distributed`, it specifies the ordinal of the current simulation process within the list parallel processes. The value must be in the range 0...n-1, where n is the number of partitions (see `parsim-num-partitions`). This option is not required when using MPI communications, because MPI has its own way of conveying this information.");
Register_GlobalConfigOption(CFGID_PARSIM_COMMUNICATIONS_CLASS, "parsim-communications-class", CFG_STRING, "omnetpp::cNamedPipeCommunications", "Selects the class that implements communication between partitions. The class must implement the `cParsimCommunications` interface. For `parsim-mode=distributed` (separate processes), use `cMPICommunications` or `cNamedPipeCommunications`; for `parsim-mode=multithreaded` (threads within one process), use `cInProcessCommunications` or `cNamedPipeCommunications`.");
