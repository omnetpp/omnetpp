...
class SIM_API ExpressionFilter : public cResultFilter
{
    ...
    protected:
        ...
        virtual void receiveSignal(cResultFilter *prev, simtime_t_cref t, long l, cObject *details) override {find(prev)->lastValue = ExprValue(l); process(t, details);}
        ...
        virtual void receiveSignal(cComponent *source, simsignal_t signalID, long l, cObject *details) override {find(source,signalID)->lastValue = ExprValue(l); process(now(), details);}
