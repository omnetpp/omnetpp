...
class StarTreeEmbedding
{
    ...
    public:
	    StarTreeEmbedding(GraphComponent *graphComponent, double leafSpacing)
	    {
	    ...
	    void calculateCenterRecursive(Vertex *vertex)
	    {
		    ...
		    else
		    {
                ...
                for (std::vector<Vertex *>::iterator it = vertex->spanningTreeChildren.begin(); it != vertex->spanningTreeChildren.end(); it++) {
                    ...
                    circles.push_back(Cc(Pt::getZero(), vertex->rs.getDiagonalLength() / 2 + leafSpacing + vertexChild->starTreeRadius));
                    ...
                    for (std::vector<Vertex *>::iterator jt = vertex->spanningTreeChildren.begin(); jt != vertex->spanningTreeChildren.end(); jt++) {
					    ...
					    circles.push_back(Cc(vertexPositioned->starTreeCenter.copy().add(vertexPositioned->starTreeCircleCenter), vertexPositioned->starTreeRadius + leafSpacing + vertexChild->starTreeRadius));
				    }
