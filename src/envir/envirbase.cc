...
Register_GlobalConfigOption(CFGID_LOG_FORMAT, "log-format", CFG_STRING, "[%l]\t", "Specifies the format string that determines the prefix of each recorded log line. Log lines can be produced by using any of the predefined C++ macros such as EV_FATAL, EV_ERROR, EV_WARN, EV_INFO, EV_DETAIL, EV_DEBUG or EV_TRACE. The format string may contain constant texts interleaved with format directives. A format directive is a '%' character followed by a single format character. See the manual for the list of available format characters.");
