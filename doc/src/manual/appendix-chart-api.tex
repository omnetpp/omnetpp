...
\begin{flushleft}
\ttt{get\_vectors(filter\_or\_dataframe="", include\_attrs=False, include\_runattrs=False, include\_itervars=False, include\_param\_assignments=False, include\_config\_entries=False, start\_time=-inf, end\_time=inf, convert\_to\_base\_unit=True)}
\end{flushleft}
...
\ttt{normalize} is true (and \ttt{cumulative} is false), the probability density
 ...
 results in the cumulative density function (CDF) being displayed.
  ...
  \item \ttt{cycle\_seed}: Alters the sequence in which colors and markers are assigned to series.
  \item \ttt{unit}: If present, it is required to be the same for all series, and it will be used in the automatic x axis label.
