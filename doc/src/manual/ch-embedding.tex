...
\fname{getModuleByPath()} method of \cclass{cSimulation}, then cast to the
specific module type via \fname{check\_and\_clast<>()} so that the getter
...
A more general way is to catch \fname{recordScalar()} method calls in the
simulation model. \cclass{cModule}'s \fname{recordScalar()} method
...
To run the simulation, the \fname{selectNextModule()} and \fname{doOneEvent}
...
\fname{selectNextModule()} may return \ttt{NULL}. The default
...
\cclass{cEnvir}: \fname{beginSend()}, \fname{sendHop()}, \fname{endSend()},
and others.
...
requires frequent use of \fname{cSimulation::set\-Active\-Simulation()}:
