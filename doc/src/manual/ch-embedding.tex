...
that delegate to the much few string-based methods (\fname{getConfigValue()}, etc.).
...
\subsection{How to manage without NED files}

it is also possible to get rid of NED files and the dependency on the nedxml library

you'll need to write \cclass{cModuleType} and \cclass{cChannelType} objects

similar to the C++ files generated by \fprog{nedtool} of {\opp} 3.x
...
\subsection{Reusing cSimulation objects}
