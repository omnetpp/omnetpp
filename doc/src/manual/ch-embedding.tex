...
\begin{itemize}
  ...
  \item{\textbf{Envir} is another library which contains all code
    that is common to all user interfaces. \ffunc{main()} is also in Envir.
...
program from scratch, i.e. starting from a \ffunc{main()} function.

\subsection{The main() Function}

Here is an minimalistic program that initializes the simulation library,
...
If network setup was successful, we call \ttt{sim->}\ffunc{startRun()}, one
side effect of which is that the \ffunc{initialize()} methods of modules
and channels get invoked. This step also results in an exception if
something goes wrong in an \ffunc{initialize()}.
...
regardless of whether there was an error, \ttt{sim->}\ffunc{endRun()} has
...
with \ffunc{setActiveSimulation(NULL)}.
...
One method that you surely want to redefine is \ffunc{readParameter()}; that is
how module parameters get their values. For debugging purposes, you may also
redefine \ffunc{sputn()} where module log messages are written.
...
and \ffunc{loadNedText()}. The first one loads a whole subdirectory tree,
...
The \ffunc{readParameter()} method should be written so that it can assign
...
It can also accept the default value if there is one (\ffunc{acceptDefault()}).
...
\subsubsection{Via C++ Calls into the Model}
...
\subsubsection{Via \cclass{cEnvir} Callbacks}

A more general way is to catch \ffunc{recordScalar()} method calls in the
simulation model. \cclass{cModule}'s \ffunc{recordScalar()} method
delegates to the similar function in \cclass{cEnvir}. You may define the
latter function such that it stores all recorded scalars (for example in an
...
A drawback is that compile-time checking of statistics names is lost, but
definite advantages include that any simulation model can now be used
...
It depends on the concrete scheduler class whether
\ffunc{selectNextModule()} may return \ttt{NULL}. The default
\cclass{cSequentialScheduler} never returns \ttt{NULL}.

Execution may terminate in various ways. Runtime errors cause a
...
such as on a simulation time limit (see above), on CPU time limit, or when
results reach a required accuracy. It is relatively straightforward to
build in progress reporting and interactivity (start/stop) as well.
...
shared among several \cclass{cSimulation} instances.
\cclass{cSimulation}'s destructor deletes the associated
\cclass{cEnvir} instance as well.

\cclass{cSimulation} instances may be reused from one simulation to another,
but it is also possible to make a new instance for each simulation run.
...
to get rid of, like the active simulation manager pointer and the active
environment object pointer. Static buffers and objects (like string pools)
are also used for efficiency reasons at some places inside the simulation
kernel.
\end{note}
...
a simulation, so \ffunc{setScheduler()} may only be called when
