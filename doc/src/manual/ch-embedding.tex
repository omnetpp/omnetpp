...
\begin{itemize}
  ...
  \item{The \textbf{Model Component Library} includes simple module definitions and 
    ...
    message types and everything belonging to that models that
    have been linked to the simulation program. A simulation program can
    run any model that contains all the required linked components.}
...
For the following section, we assume that you write the embedding
...
\ttt{ExecuteOnStartup::executeAll()} performs various startup tasks such as,
...
to read the configuration. In addition, the results produced by the sim. manager are written to this environment object.
...
the \ffunc{initialize()} methods of modules
and channels are then activated. In addition, an exception is thrown if
something goes wrong in the \ffunc{initialize()}.
...
You must redefine is \ffunc{readParameter()} method. This enables 
...
as the active simulation manager. This is necessary since several models
...
to get rid of, such as the active simulation manager pointer and the active
