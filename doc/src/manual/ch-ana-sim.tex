...
\begin{itemize}
  ...
  \item\tbf{lastval}: record the last value
  ...
  \item\tbf{stddev}: record the count, sum, mean, standard deviation, minimum
      and maximum of the values
  \item\tbf{histogram}: record everything \ttt{stddev} records, plus a histogram
...
\ttt{vector-recording-interval=} per-object options. The syntax of the
configuration options are
\textit{<module-full-path>.<vector-name>.}\ttt{vector-recording=}\textit{true/false},
and \textit{<module-full-path>.<vector-name>.}\ttt{vector-recording-interval=}\textit{<intervals>},
...
For the \ttt{vector-recording-interval=} option, one can specify
...
A third per-vector configuration option is \ttt{vector-record-eventnumbers=},
...
using the \ttt{output-scalar-precision=} and \ttt{output-vector-precision=}
...
usually smaller than the one that the \ttt{double} variable already had
...
The appropriate configuration options are \ttt{outputvectormanager-class=}
and \ttt{outputvectormanager-class=}. For example,
