...
First, we obtain the properties of the bar chart from the \ttt{chart} module.
The \ttt{props} object we get is a Python \ttt{dict} whose entries serve as
...
print(repr(k), "=", repr(v))} for fancier output. When the chart script runs,
...
Try \ttt{print(df)} to print the dataframe contents. You'll get something like
...
(\ttt{plot.set\_warning()} displays the message in the plot area).
...
for them. This is done with \ttt{utils.pick\_two\_columns(df)}.
...
For the bar chart, we want values to appear on the horizontal axis and in the
...
lexicographical sorting done by \ttt{df.sort\_values()} is not the best
...
If you add a \ttt{print(df)} statement, you can see the result of pivoting:
...
groups can be accessed as \ttt{df["mean"]} and \ttt{df["conf\_intv"]} as separate
dataframes. This is how they are passed into \ttt{utils.plot\_bars()}, which
...
\begin{figure}[htbp]
  \begin{center}
    \includesvg[scale=0.7]{figures/ana-barplot-errorbars}
...
% utils.preconfigure_plot(props)
% filter_expression = props["filter"]
% df = results.get_...(filter_expression, ...)
% ...
% utils.plot_...(...)
% utils.postconfigure_plot(props)
% utils.export_image_if_needed(props)
