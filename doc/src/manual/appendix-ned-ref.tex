...
A compound module's internal structure is defined by
its submodules (\ref{ch-ned-ref:sec:submodules}) and
connections (\ref{ch-ned-ref:sec:connections}).

Compound module types may also have inner types (\ref{ch-ned-ref:sec:inner-types})
...
Submodules can be scalar or vector, the vector size being specified with
a numeric expression inside the square brackets.
...
body might be present or missing. missing is same as empty. If present,
may contain parameters and gates.

Extra params CANNOT be added in submodules -- they can only be added in
new module types.
...
FIXME a simple module may also be a network!
...
gates (\ref{ch-ned-ref:sec:gates}). However, parameters cannot have
a value assigned, not even a default value.
...
parameters (\ref{ch-ned-ref:sec:parameters}). However, parameters cannot have
a value assigned, not even a default value.
...
TODO syntax,
structure
what can have properties, where
...
of the component type, submodule or connection. The \fkeyword{parameters}
keyword is optional, and can be omitted without change in the meaning.
...
\subsubsection{Recognized parameters properties}
