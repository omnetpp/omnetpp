...
If an item in a valuelist contains a comma or semicolon, the item
must be quoted (using double quotes) to prevent the comma or semicolon
from being interpreted as separator. A quotation mark must be escaped
with a backslash.

FIXME verify this whole quoting business!!!! e.g. is \, also OK?

\begin{verbatim}
@prop(marks=the ! mark, "the , mark", "the ; mark", the \" mark);
\end{verbatim}

One layer of quotes is removed from value items automatically, that is,
\ttt{foo} and \ttt{"foo"} are the same. To add a value item with
quotation marks included, enclose it in an extra layer of quotes:
\ttt{"\textbackslash"foo\textbackslash""}.
