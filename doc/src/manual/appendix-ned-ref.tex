...
\begin{note}
    Note that namespaces coming from multiple \ttt{@namespace} properties
...
\ttt{@directIn} should be used when the gate is an \fkeyword{input} gate that is
...
\begin{note}
    The reason \ttt{@directIn} gates are not \textit{required} to remain
    unconnected is that it is often useful to wrap such modules in a compound
    module, where the compound module also has a \ttt{@directIn} input gate
