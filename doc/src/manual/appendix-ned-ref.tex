...
An identifier in an expressions occurring anywhere in a compound module
(including submodule bodies) is interpreted as referring to a parameter
of the compound module, unless it is a connection \ttt{for} loop variable.

TODO in other contexts?

In submodule bodies, parameters of the same submodule must be referred to
...
\subsection{The \fname{index} and \fname{sizeof()} operators}

The \ttt{index} operator used in a vector submodule's body yields the index
of the submodule instance. When used in a non-vector submodule's body,
it evaluates to zero. \ttt{index} should not be used in other contexts.

The \fname{sizeof()}\index{ned!sizeof()} operator returns the size
of a submodule vector or a gate vector. The interpretation of the
argument to \ttt{sizeof()} depends on the context where the
expression occurs.
