...
\ttt{@directIn} should be used when the gate is an inut gate that is
...
(\ttt{connections allowunconnected:}), gates can be left unconnected.

\begin{note}
    The \ttt{directIn} and \ttt{@loose} gate properties are to the
    \ttt{connections allowunconnected} syntax; see \label{ch-ned-ref:sec:gates}.
\end{note}

\subsubsection{Single and group connections}

TODO


\subsubsection{Specifying gates}

if module is missing, that means the compound module.

\ttt{$i} / \ttt{$o} notation ("subgate")
