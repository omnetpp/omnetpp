...
Expressions have a C-style syntax. Elements: parameters, loop variables,
functions, operators.

TODO volative vs nonvolatile:

For volatile parameters, the expression is evaluated every time the
parameter value is accessed. Non-volatile parameters are evaluated only once.

const subexpression (not yet supported)
...
stack types: bool, double, string, xml

Numeric values are represented as \ttt{double}s.

For bitwise operators, \ttt{double}s are converted to \ttt{unsigned long}
using the C/C++ builtin conversion (type cast), the operation is performed,
then the result is converted back to \ttt{double}.

For modulus (\%), the operands are converted to \ttt{long}.

no implicit conversion between bool and numeric types! nor between string + numeric

TODO @unit and expression evaluation wrt units
