...
to be used. For a submodule, if all gates of the given gate vector are connected,
the gate vector gets expanded by one. For the compound module, it causes an error
if all gates of the given gate vector are connected.

\begin{note}
    Due to the top-down order of network building, it is not possible to
    expand the gate vector of the compound module: the new gates would be
    left unconnected on the outside.
\end{note}
...
Regarding gate vectors, the two sides of the gate \ttt{gate\$i[]} and \ttt{gate\$o[]}
must be equal. Therefore, it is not possible to use the \ttt{gate++} operator
with \ttt{gate\$i} and \ttt{gate\$o}.
