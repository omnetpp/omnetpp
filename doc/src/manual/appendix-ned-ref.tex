...
\begin{note}
You can print the full definitions by running \ttt{opp\_run -h neddecls}.
\end{note}
...
For compound modules, the built-in default compound module C++ class
\cclass{cCompoundModule} will be used, unless the module contains the
\fprop{@class} property. When \fprop{@class} is present, the resolution
...
(see \ref{ch-ned-ref:sec:literals}).
...
(\ref{ch-ned-ref:sec:pattern-assignments}) and properties
...
\ref{ch-ned-ref:sec:connection-groups}).
...
\label{ch-ned-ref:sec:connection-groups}
...
\begin{note}
New NED functions can be defined in C++; see section \ref{sec:ch-sim-lib:defining-ned-functions}.
\end{note}
...
\begin{note}
Integers are represented with the C++ type \ttt{long}. \ttt{double}-to-\ttt{long}
conversions are performed using the C++ language's built-in typecast operator.
As \ttt{long} is 32 bits on most 32-bit architectures, for portability reasons
