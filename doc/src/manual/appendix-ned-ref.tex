...
Section \ref{ch-ned-lang:sec:units} lists the units recognized by \opp.
...
\subsection{The C++ class name}

The C++ implementation class for a simple module is determined as follows:

If the simple module extends another simple module, and has no \ttt{@class}
property, the C++ implementation class is inherited from the base class.

If the simple module contains has a \ttt{@class} property, the C++ class
...
If the simple module contains no \ttt{@class} property and has no base
class, the C++ class name will be composed of the \textit{current namespace} and the
unqualified name simple module type.

\begin{note}
    NED subclassing does not imply subclassing the C++ implementation. If you
    subclass a simple module in NED as well as in C++, you explicitly need the to
    specify the \ttt{@class} property, otherwise the derived simple module
    will continue to use the C++ class from its super type.\end{note}
...
When a channel does not explicitly extend another channel, the implicit
base class is \ttt{ned.BasicChannel}, which already declares
the \ttt{delay}, \ttt{error} and \ttt{datarate} parameters (see
\ref{ch-ned-ref:sec:built-in-defs}). 

To create a channel that does not have these parameters, the channel needs to
extend \ttt{ned.IdealChannel}.

\subsection{The C++ class name}

The C++ implementation class for a simple module is determined as follows:

If the channel does not have a \ttt{@class} property, the C++ class gets
inherited from the base class (or from the implicit base class
\ttt{ned.BasicChannel}).

If the channel has a \ttt{@class} property, the C++ class
name will be composed of the \textit{current namespace} (see
\ref{cha:ned-ref:current-namespace}) and the value of the \ttt{@class}
property. The \ttt{@class} property should contain a single value.

\begin{note}
    1. As with simple modules, the \ttt{@class} property is needed when you want
    to subclass the C++ implementation class as well.
\end{note}
    
\begin{note}
    2. It was considered to make \ttt{IdealChannel} the default base
    class and let the user spell out \ttt{extends BasicChannel}, but this idea
...
package: channels \ttt{IdealChannel} and \ttt{BasicChannel}, and module
