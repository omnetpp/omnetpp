...
(such as the \fconfig{load-libs}, \fconfig{output-vector-file}, \fconfig{output-scalar-file}
...
explicitly, using the \fconfig{extends} key. Consider the following ini file
skeleton:
...
\begin{itemize}
  \item{if \ttt{A} does not have an \fpar{extends} key then
        \ttt{A}, \ttt{General}}
  \item{otherwise the merge of the configurations enumerated in
        the \fpar{extends} key, and all of their
...
\begin{note}
    ...
    things like \ttt{\$\{j=\$i..10\}}), although it might be implemented in
    future {\opp} releases.
\end{note}
