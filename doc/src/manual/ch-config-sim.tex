...
\ffilename{omnetpp.ini} entry, and as long as every module, channel etc
...
\item If the first match is a value line (matches \textit{<parameter-fullpath>}\ttt{=somevalue}),
...
requested by the user, by configuring param-record-as-scalar=true for the
parameters in question. The configuration key is a pattern that
identifies the parameter, plus ".param-record-as-scalar". An example:
...
param-record-as-scalar=true is not needed because iteration variables are
