...
The \textit{<parameter-fullpath>}\ttt{.apply-default=true} setting assigns
the default value to the parameter if it has one.

The \textit{<parameter-fullpath>}\ttt{.apply-default=false} setting prohibits
assigning the default value to the parameter.

If a parameter was not set but has a default value, that value will be
assigned. This is like having a \ttt{**.apply-default=true} line at the
...
\item Then the first entry matching \textit{<parameter-fullpath>} or
    \textit{<parameter-fullpath>}\ttt{.apply-default} is found in the configuration.
    When a \textit{<parameter-fullpath>}\ttt{.apply-default=false} line is found,
    the search continues but further \ttt{apply-default}
    lines will be ignored; this effectively prohibits accepting the
    the default value. If the parameter has no default value,
    \ttt{apply-default} lines are ignored from the start.
\item If the first match is a value line (matches \textit{<parameter-fullpath>}),
    the value is applied and the process finishes.
\item If the first match is an \ttt{apply-default=true} line,
    the default value is applied and the process finishes.
\item If there was no match and no \ttt{apply-default=false} line has been
    encountered either, and the parameter has a default value, it is applied
...
each run. Note that the parameter study actually maps to nested
...
\begin{Verbatim}[commandchars=\\\{\}]
\$ aloha -u Cmdenv -c AlohaStudy -n -g
{\opp} Discrete Event Simulation
...
Config: AlohaStudy
Number of runs: 24
...
The "enabled" and "interval" words changed to "vector-recording" and
...
\begin{Verbatim}[commandchars=\\\{\}]
**.fifo[2].queueLengthVector.vector-recording = false
**.fifo[*].queueLengthVector.vector-recording-interval = ..100, 300..500, 900..
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
\textit{module-pathname}.\textit{objectname}.vector-recording = true/false
\textit{module-pathname}.\textit{objectname}.vector-recording-interval = \textit{start1}..\textit{stop1}, \textit{start2}..\textit{stop2}, ...
\end{Verbatim}
