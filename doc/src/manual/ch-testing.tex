...
of simulation events, messages and statistics. The hash value is continuosly
...
just needs to compare the computed fingerprints to that from a reference run
...
{\opp} will compare the computed fingerprints with the provided ones, and
if they differ an error is generated.

The fingerprint computation algorithm allows controlling what is included in
the hash value by providing the \fconfig{fingerprint-ingredients} option. This
option specifies a subset of the set of predefined properties of simulation
events, messages and statistics. Some notable examples are: simulation time,
event number, module full path, message length, message content, scalar result,
vector result. Changing the ingredients allows to make the fingerprint sensitive
for certain changes while keeping it immune to other changes. If this is not
enough, the algorithm also supports computing multiple different fingerprints
simultaneously.
  \footnote{The old ({\opp} 4.x) fingerprint was computed from the module
  ID and simulation time of each event. The same fingerprint can still be
  computed by setting the \fconfig{fingerprint-ingredients} option to
  \ttt{"ti"}. }

In addition, independently of the ingredients, the \fconfig{fingerprint-events},
\fconfig{fingerprint-modules}, and \fconfig{fingerprint-results} options can
be used to further filter for events, modules, and statistical results
respectively. These options take a wildcard expression that is used to match
against the corresponding object before including its property in the fingerprint.
These filters are mainly useful to compute fingerprints only for a subset of the
simulation.

\begin{warning}
The computed fingerprint value may be dependent on the accuracy of the
floating point arithmetic. There are differences between the floating point
operations of AMD and Intel CPUs. Running under a processor emulator software
...
expected fingerprints), and reports the results. The script is extensively
