...
\section{Smoke Tests}
\label{ch-testing:sec:smoke-tests}
...
\label{ch-testing:sec:fingerprint-tests}
...
\cclass{cFingerprint} is the class responsible for fingerprint computation.
The current fingerprint computation object can be retrieved from
\cclass{cSimulation}, using the \ffunc{getFingerprint()} member function.
This method will return \ttt{nullptr} if fingerprint computation is turned
off for the current simulation run.

To contribute data to the fingerprint, \cclass{cFingerprint} has several
...
\begin{cpp}
if (cFingerprint *fingerprint = getSimulation()->getFingerprint()) {
...
\section{Unit Tests}
\label{ch-testing:sec:unit-tests}
...
\label{ch-testing:sec:module-tests}
...
\label{ch-testing:sec:statistical-tests}
...
\label{ch-testing:sec:statistical-regression-tests}
