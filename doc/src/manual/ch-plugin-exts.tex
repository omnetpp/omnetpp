...
\begin{itemize}
   ...
   \item \cclass{cFutureEventSet}. This extension interface allows one to
     ...
     workloads.
   \item \cclass{cFingerprint}. This extension interface allows one
...
\cclass{cEvent} represents an event in the discrete event simulator. (The
...
from the FES one by one, and their \ffunc{execute()} member functions are
called. \ffunc{execute()} should be overridden in subclasses to carry out
the actions associated with the event. For example, \cclass{cMessage} is
also a subclass of \cclass{cEvent}, where \ffunc{execute()} ends up calling
the \ffunc{handleMessage()} method of the destination module, or switches
to the coroutine of its \ffunc{activity()} method.
...
% TODO \cclass{cFingerprint}

The fingerprint computation class must implement the \cclass{cFingerprint} interface,
and can be activated with the \fconfig{fingerprint-class} configuration option.
...
the \cclass{cIEventlogManager} interface, and can be activated with the
\fconfig{eventlogmanager-class} configuration option.

The default eventlog manager is \cclass{EventlogFileManager}.
...
\ttt{-u} \textit{<name>} command-line or the \fconfig{user-interface}
