...
(see \ttt{src/sim/bootenv.cc} in the source code):

\begin{enumerate}
  \item First, ini files (specified as command-line arguments,
     or \ttt{omnetpp.ini} if none were specified) are read into a
     \textit{boot-time configuration object}.
  \item Shared libraries get loaded (see the \ttt{-l} command-line option,
     and the \ttt{load-libs} configuration option)
  \item The \ttt{configuration-class} configuration option is examined.
     ...
     The new configuration object receives the boot-time configuration
     when it gets initialized, so it can take parameters (e.g. database
     connection parameters, XML file name, etc) from the boot-time
     config. Then the boot-time configuration object is deallocated.
