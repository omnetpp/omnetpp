...
\item[cmdenv-output-file] = \textit{<filename>}\\
    ...
    \ttt{fname-{\allowbreak}append-{\allowbreak}host} option as well.
...
\item[cmdenv-runs-to-execute] = \textit{<string>}\\
    ...
    comma-separated list of run numbers or run number ranges, e.g.
...
\item[eventlog-file] = \textit{<filename>}, default: \ttt{\$\{{\allowbreak}resultdir\}{\allowbreak}/{\allowbreak}\$\{{\allowbreak}configname\}{\allowbreak}-{\allowbreak}\$\{{\allowbreak}runnumber\}{\allowbreak}.{\allowbreak}elog}\\
    \textit{Per-simulation-run setting.}\\
...
\item[output-scalar-file] = \textit{<filename>}, default: \ttt{\$\{{\allowbreak}resultdir\}{\allowbreak}/{\allowbreak}\$\{{\allowbreak}configname\}{\allowbreak}-{\allowbreak}\$\{{\allowbreak}runnumber\}{\allowbreak}.{\allowbreak}sca}\\
    \textit{Per-simulation-run setting.}\\
...
\item[output-vector-file] = \textit{<filename>}, default: \ttt{\$\{{\allowbreak}resultdir\}{\allowbreak}/{\allowbreak}\$\{{\allowbreak}configname\}{\allowbreak}-{\allowbreak}\$\{{\allowbreak}runnumber\}{\allowbreak}.{\allowbreak}vec}\\
    \textit{Per-simulation-run setting.}\\
...
\item[output-vector-precision] = \textit{<int>}, default: \ttt{14}\\
    ...
    file. The maximum value is {\textasciitilde}15 (IEEE double precision).
    This setting has no effect on the "time" column of output vectors, which
    are represented as fixed-point numbers and always get recorded precisely.
...
\item[qtenv-extra-stack] = \textit{<double>}, unit=\ttt{B}, default: \ttt{48Ki\-B}\\
    \textit{Global setting (applies to all simulation runs).}\\
    Specifies the extra amount of stack that is reserved for each
...
\item[snapshot-file] = \textit{<filename>}, default: \ttt{\$\{{\allowbreak}resultdir\}{\allowbreak}/{\allowbreak}\$\{{\allowbreak}configname\}{\allowbreak}-{\allowbreak}\$\{{\allowbreak}runnumber\}{\allowbreak}.{\allowbreak}sna}\\
    \textit{Per-simulation-run setting.}\\
...
\item[**.vector-max-buffered-values] = \textit{<int>}\\
    ...
    \ttt{<module-{\allowbreak}full-{\allowbreak}path>.{\allowbreak}<vector-{\allowbreak}name>.{\allowbreak}vector-{\allowbreak}max-{\allowbreak}buffered-{\allowbreak}values={\allowbreak}<count>}.
...
\item[\$\{iterationvars2\}] : \\
    Concatenation of all user-defined iteration variables in
    \ttt{name={\allowbreak}value} form, plus
    \ttt{\$\{{\allowbreak}repetition\}{\allowbreak}}
\end{description}
