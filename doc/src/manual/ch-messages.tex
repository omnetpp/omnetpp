...
\fname{setByteLength()} sets the same length field as \fname{setLength()},
...
\fname{byteLength()} also reads the length field as \fname{length()},
...
TCP OPEN, SEND, CLOSE, etc), the message kind field (\ttt{kind()},
...
\fname{encapsulatedMsg()} should be viewed as if it returned a
...
derived from \fname{cOwnedObject}, thus it will not have \fname{name()},
