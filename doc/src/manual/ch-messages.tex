...
\ffunc{setByteLength()} sets the same length field as \ffunc{setBitLength()},
...
\ffunc{getByteLength()} also reads the length field as \ffunc{length()},
...
of the original, including message parameters (\cclass{cMsgPar} or other
...
bool \ffunc{isSelfMessage()};
bool \ffunc{isScheduled()};
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
simtime_t \ffunc{getCreationTime()}
simtime_t \ffunc{getSendingTime()};
simtime_t \ffunc{getArrivalTime()};
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
int \ffunc{getSenderModuleId()};
int \ffunc{getSenderGateId()};
int \ffunc{getArrivalModuleId()};
int \ffunc{getArrivalGateId()};
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
cModule *\ffunc{getSenderModule()};
cGate *\ffunc{getSenderGate()};
cGate *\ffunc{getArrivalGate()};
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
bool \ffunc{arrivedOn}(int id);
bool \ffunc{arrivedOn}(const char *gname, int gindex=0);
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
void \ffunc{setControlInfo(cObject *controlInfo)};
cObject *\ffunc{getControlInfo()};
cObject *\ffunc{removeControlInfo()};
\end{Verbatim}
...
The \ffunc{encapsulate()} function encapsulates a message
...
You can get back the encapsulated message by \ffunc{decapsulate()}:

\begin{verbatim}
cMessage *userdata = tcpseg->decapsulate();
\end{verbatim}

\ffunc{decapsulate()} will decrease the length of the message accordingly,
...
encapsulated messages, meaning that if you \ffunc{dup()} a message
...
the encapsulated message is deferred until \ffunc{decapsulate()}
...
large memory and execution time overhead. \ffunc{cMsgPar}'s are
...
Using \cclass{cMsgPar}s is also error-prone because \ffunc{cMsgPar} objects
...
If you still need \cclass{cMsgPar}s for some reason, here's a short summary.
...
\ttt{mypacket\_m.h} contains the declaration of the \ffunc{MyPacket} C++ class, and
...
The \ttt{mypacket\_m.cc} file contains implementation of the generated \ffunc{MyPacket}
...
linked into your simulation. (If you use the \ffunc{opp\_makemake} tool
...
An \ffunc{enum \{..\}} generates a normal C++ enum, plus creates an object
...
a C++ header file with a generated class, \ffunc{FooPacket}.
\ffunc{FooPacket} will be a subclass of \ffunc{cMessage}.
...
methods will begin with \ffunc{get} and \ffunc{set},
followed by the field name with its first letter converted to uppercase.
Thus, \ffunc{FooPacket} will contain the following methods:
...
Note that the methods are all declared \ffunc{virtual} to give you the possibility
...
Data types for fields are not limited to \ffunc{int} and \ffunc{bool}. You can use the
following primitive types (i.e. primitive types as defined in the C++ language):

\begin{itemize}
   \item \ffunc{bool}
   \item \ffunc{char}, \ffunc{unsigned char}
   \item \ffunc{short}, \ffunc{unsigned short}
   \item \ffunc{int}, \ffunc{unsigned int}
   \item \ffunc{long}, \ffunc{unsigned long}
   \item \ffunc{double}
\end{itemize}
...
You can declare that an \ffunc{int} (or other integral type) field
...
The generated getter and setter methods will have an extra \ffunc{k} argument,
the array index:
...
The generated getter and setter methods will return and accept \ffunc{const char*}
pointers:
...
(\ffunc{int}, \ffunc{double}, \ffunc{char}, ...) to \ffunc{cMessage}.
...
\begin{itemize}
  \item set up a hierarchy of message (packet) classes, that is,
    not only subclass from \ffunc{cMessage} but also from your
    own message classes;
...
By default, messages are subclassed from \ffunc{cMessage}. However, you can
explicitly specify the base class using the \ffunc{extends} keyword:
...
Until now we have used the \ffunc{message} keyword to define classes, which
implies that the base class is \ffunc{cMessage}, either directly or indirectly.

But as part of complex messages, you'll need structs and other classes
(rooted or not rooted in \ffunc{cOwnedObject}) as building blocks.
Classes can be created with the \ffunc{class} class keyword;
structs we'll cover in the next section.

The syntax for defining classes is almost the same as defining messages,
only the \ffunc{class} keyword is used instead of \ffunc{message}.

Slightly different code is generated for classes that are rooted in
\ffunc{cOwnedObject} than for those which are not.
If there is no \ttt{extends}, the generated class will not be
derived from \ffunc{cOwnedObject}, thus it will not have \ffunc{getName()},
...
as a field. For example, if you have a struct named \ffunc{IPAddress},
...
The \ffunc{IPAddress} structure must be known in advance to the message compiler;
...
The generated class will contain an \ffunc{IPAddress} data member
(that is, \textbf{not} a pointer to an \ffunc{IPAddress}).
...
Suppose you have an \ffunc{IPAddress} structure, defined in an \ttt{ipaddress.h}
file:
...
To be able to use \ffunc{IPAddress} in a message definition, the message
...
The effect of the first three lines is simply that the \ffunc{\#include}
statement will be copied into the generated \ttt{foopacket\_m.h}
file to let the C++ compiler know about the \ffunc{IPAddress} class.
...
\ffunc{IPAddress} is a C++ struct. This information will (among others)
affect the generated code.

Classes can be announced using the \ffunc{class} keyword:

\begin{Verbatim}[commandchars=\\\{\}]
\tbf{class} cSubQueue;
\end{Verbatim}

The above syntax assumes that the class is derived from \ffunc{cOwnedObject}
either directly or indirectly. If it is not, the \ffunc{noncobject}
...
The distinction between classes derived and not derived from \ffunc{cOwnedObject}
is important because the generated code differs at places.
The generated code is set up so that if you incidentally
forget the \ffunc{noncobject} keyword (and thereby mislead the
message compiler into thinking that your class is rooted in
\ffunc{cOwnedObject} when in fact it is not), you'll get a C++ compiler
...
For example, when setting a integer field named \ffunc{payloadLength},
you might also need to adjust the packet length. That is,
the following default (generated) version of the
\ffunc{setPayloadLength()} method is not suitable:
...
the generated code will contain a \ffunc{FooPacket\_Base} class
instead of \ffunc{FooPacket}. Then you would subclass
\ffunc{FooPacket\_Base} to produce \ffunc{FooPacket},
...
for \ffunc{FooPacket}, because not everything can be
pre-generated as part of \ffunc{FooPacket\_Base}, e.g.
...
in a single \ffunc{int} or \ffunc{short}, and still you want
...
For an \ffunc{abstract} field, the message compiler generates
...
\begin{itemize}
 \item \ttt{dynapacket.msg} defines \ffunc{DynaPacket} and \ffunc{DynaDataPacket};
 \item \ttt{dynapacket\_m.h} and \ttt{dynapacket\_m.cc} are produced
   by the message subclassing compiler from it, and they contain
   the generated \ffunc{DynaPacket} and \ffunc{DynaDataPacke}t
...
\cclass{cMessage} to get a new message class. You could write the following:
  ...
  In real code, \ffunc{freq} and \ffunc{power} should be private members,
...
Tkenv for debugging. While \ffunc{cMsgPar}-based message parameters can be viewed in
...
\ffunc{RadioMsg} fields. This descriptor class might look like this:
...
Then you have to inform Tkenv that a \ffunc{RadioMsgDescriptor} exists and that it
should be used whenever Tkenv finds messages of type \ffunc{RadioMsg} (as it is
currently implemented, whenever the object's \ffunc{getClassName()} method returns
\ttt{"RadioMsg"}). So when you inspect a \ffunc{RadioMsg} in your simulation, Tkenv
can use \ffunc{RadioMsgDescriptor} to extract and display the values of
the \ffunc{freq} and \ffunc{power} variables.
