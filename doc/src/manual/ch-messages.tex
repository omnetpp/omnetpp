...
This is \textbf{\textbf{not:}}

\begin{itemize}
  \item{\textit{... a preprocessor to C++.} You cannot (and you're not meant to)
     write C++ statements into \ttt{.msg} files. It defines data only -- not any
     kind of active behaviour.}
  \item{\textit{... an attempt to reproduce the functionality of C++ with another
     syntax.} Do not look for templates, conditional compilation, etc.}
...
\begin{itemize}
  \item{\textit{Why no support for \ttt{std::vector} and other STL classes?}
     Message definitions focus on the interface
     (getter/setter methods) of the classes, while STL belongs to the
     implementation (STL purposefully wants to provide \textit{nuts and bolts}
     for C++ programs).
     Actually you \textit{can} use STL vectors: you can specify
     array fields of dynamic size as abstract, and you can provide an
     \ttt{std::vector}-based implementation by customizing the class.}
