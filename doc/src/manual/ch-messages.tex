...
large memory and execution time overhead. \fname{cMsgPar}'s are
...
Using \cclass{cMsgPar}s is also error-prone because \fname{cMsgPar} objects
...
with the \fname{addPar()} member function, then set its value with
one of the methods \fname{setBoolValue()}, \fname{setLongValue()},
\fname{setStringValue()}, \fname{setDoubleValue()}, \fname{setPointerValue()},
\fname{setObjectValue()}, and \fname{setXMLValue()}. There are also overloaded
...
by name and obtain a reference to it with the \fname{par()} member function.
\fname{hasPar()} can be used to check first whether the message object has a parameter
object with the given name. Then the value can be read with the methods
\fname{boolValue()}, \fname{longValue()}, \fname{stringValue()},
\fname{doubleValue()}, \fname{pointerValue()}, \fname{objectValue()},
\fname{xmlValue()}, or by using the provided overloaded type cast operators.
...
Tkenv for debugging. While \fname{cMsgPar}-based message parameters can be viewed in
