...
\begin{itemize}
    ...
    \item \ttt{-O <directory>}, \ttt{-{}-out <directory>} :
    ...
    \item \ttt{-X <directory>}, \ttt{-X<directory>}, \ttt{-{}-except <directory>} :
                          With \ttt{-r} or \ttt{-{}-deep}: ignore the given directory.
    \item \ttt{-d<subdir>}, \ttt{-d <subdir>}, \ttt{-{}-subdir <subdir>} :
                          Causes make to switch to the given directory and invoke
                          a Makefile in that directory.
    \item \ttt{-n, -{}-nolink} : Produce object files but do not create an executable or library.
    \item \ttt{-s, -{}-make-so} : Build shared library (\ttt{.so}, \ttt{.dll} or \ttt{.dylib}).
    \item \ttt{-a, -{}-make-lib} : Create static library (\ttt{.a} or \ttt{.lib}).
    \item \ttt{-I<dir>} : Add the given directory to the C++ include path.
    \item \ttt{-L<dir>} : Add the given directory to the library path.
    \item \ttt{-l<library>} : Additional library to link against.
\end{itemize}
...
\begin{itemize}
  \item Use the \ttt{-I<dir>} option to specify the location of
        ...
        on Linux.
  \item Use the \ttt{-L<dir>} to specify the location of the binaries
  ...
  \item Use the \ttt{-l<libname>} to specify the name of the library.
...
and options using the \ttt{-h} option.
...
header file instead of passing them to the compiler via \ttt{-D} options.
...
At the same time, \ttt{-D} options must be removed from the compiler
