...
\fprog{opp\_makemake} automatically adds dependencies to the
Makefile\index{Makefile!dependencies}. It also adds a \ttt{depend} target
that regenerates the dependency information in the Makefile, i.e. one needs
to run \ttt{make depend} when dependency information might be out of date.

\ttt{make depend} internally relies on the \fprog{opp\_makedep} tool. Run
\fprog{opp\_makedep -{}-help} to display usage information.
