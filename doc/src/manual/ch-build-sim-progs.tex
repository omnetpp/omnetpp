...
would be something like \ttt{libopptkenv.a} (or \ttt{libopptkenv.a.}\textit{<version>}),
and the shared library would be called \ttt{libopptkenv.so}
(or \ttt{libopptkenv.so.}\textit{<version>}).
The Windows version of the static library would be \ttt{opptkenv.lib},
and the DLL would be named \ttt{opptkenv.dll}.

You'll need to link with the following libraries:

\begin{itemize}
  \item{The simulation kernel and class library\index{simulation!kernel},
    called \textit{oppsim} (file \ttt{liboppsim.a}, \ttt{oppsim.lib}, etc).}
  \item{User interfaces. The common part of all user interfaces is
    the \textit{oppenvir} library (file \texttt{liboppenvir.a}, etc),
    and the specific user interfaces are \textit{opptkenv} and \textit{oppcmdenv}
    (\ttt{libopptkenv.a}, \ttt{liboppcmdenv.a}, etc). You have to link
    with \textit{oppenvir}, plus either \textit{opptkenv} or \textit{oppcmdenv}.}
\end{itemize}
