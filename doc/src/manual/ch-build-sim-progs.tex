...
to be translated into C++ using the message compiler (\fprog{opp\_msgc}).
...
The \ttt{contrib} directory depends on \ttt{mfcore} so we use the \fopt{-L} and \fopt{-l} options
...
\item Compile options, for example \fopt{-DWITH\_IPv6}. When the feature is enabled, the compiler
    ...
    that only compiles when a given feature is enabled. Currently only the \fopt{-D} option
