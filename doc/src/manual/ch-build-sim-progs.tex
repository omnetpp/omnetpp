...
(\ttt{.o} files (using gcc on Mac, Linux) or mingw on Windows)\ifcommercial or \ttt{.obj} (using MSVC on Windows)) \fi
...
\begin{itemize}
    ...
    \item{\ttt{-n, -\--nolink} :  Produce object files but do not create executable or
                          library.}
    \item{\ttt{-s, -\--make-so} : Build shared library (.so or .dll).}
...
\fprog{opp\_makemake} generates a makefile that can create both relase and debug builds.
...
options for \fprog{opp\_makemake} when generating your makefiles.
\begin{verbatim}
...
source folder not in the output folder.
\end{note}
...
\ttt{-\--nolink} option, exclude every subdirectory from the bild (-X.) and explicitly call
