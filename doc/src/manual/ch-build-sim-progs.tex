...
\ttt{opptkenv.lib}, and the DLL would be named \ttt{opptkenv.dll}.

NOTE: On Mac OS X shared libraries have the \ttt{.dylib} extension instead of \ttt{.so}.
...
NOTE: You may turn off the include autodiscovery mechanism with the \texttt{--no-deep-includes} option.
...
\begin{itemize}
  \item{You should use \ttt{opp_nmakemake} instead of \ttt{opp_makemake}}
