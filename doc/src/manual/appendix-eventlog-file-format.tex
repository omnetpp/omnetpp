...
\tbf{SB} \textit{(SimulationBegin)}: recorded at the first event
...
\tbf{SE} \textit{(SimulationEnd)}: optional last line of an event log file
...
\begin{itemize}
  ...
  \item \tbf{n} (\textit{fullName}, string): full dotted hierarchic module name
  \item \tbf{cm} (\textit{compoundModule}, bool): simple or compound module
\end{itemize}
...
\begin{itemize}
  ...
  \item \tbf{o} (\textit{isOutput}, bool): input or output gate
\end{itemize}
...
\tbf{E} \textit{(Event)}: an event that is processing of a message
...
\begin{itemize}
  ...
  \item \tbf{msg} (\textit{messageId}, long): life time unique id of the message being processed
\end{itemize}

\tbf{CE} \textit{(CancelEvent)}: canceling an event caused by self message

\begin{itemize}
  \item \tbf{id} (\textit{messageId}, long): id of the message being removed from the FES
  \item \tbf{pe} (\textit{previousEventNumber}, eventnumber\_t): event number from which the message being cancelled was sent or -1 if the message was sent from initialize
\end{itemize}

\tbf{BS} \textit{(BeginSend)}: beginning to send a message

\begin{itemize}
  \item \tbf{id} (\textit{messageId}, long): life time unique id of the message being sent
  ...
  \item \tbf{n} (\textit{messageFullName}, string): message name
  \item \tbf{pe} (\textit{previousEventNumber}, eventnumber\_t): event number from which the message being sent was processed or -1 if the message has not yet been processed before
  ...
  \item \tbf{er} (\textit{hasBitError}, bool): true indicates the message has bit errors
...
\begin{itemize}
  \item \tbf{id} (\textit{messageId}, int): id of the message being deleted
  \item \tbf{pe} (\textit{previousEventNumber}, eventnumber\_t): event number from which the message being deleted was sent or -1 if the message was sent from initialize
\end{itemize}
