...
FIXME enumerating parameters etc.
...
parameter is assigned a random value like \ttt{exponential(1.0)}, it will get
...
\subsection {stringValue() vs stdstringValue()}

\cclass{cPar} contains two methods for returning string value:
\fname{stringValue()} that return \ttt{const char *}, and
\fname{stdstringValue()} which returns \ttt{std::string}.
For volatile parameters, only the second one may be used...FIXME
...
its \fname{handleParameterChange()} method.   %%FIXME add link
...
The \fname{str()} method converts the value to a string; if the parameter...
%    cProperties *getProperties() const;
%    const char *getUnit() const;

FIXME Further methods....  Expressions etc...
