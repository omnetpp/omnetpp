...
line would ensure that when you create an instance of SlidingWindowProtocol in
your NED files, the module has the parameters and gates given in the
\ttt{SlidingWindowProtocol} NED declaration, and the implementation will be
an instance of the \ttt{SlidingWindowProtocol} C++ class.
...
Module declarations should not be put into header files, because they
are macros expanding to lines for which the compiler generates code.
...
{
  public:
    SlidingWindowProtocol(const char *name, cModule *parentmodule,
...
{
  public:
    Module_Class_Members(TransportProtocol, cSimpleModule, 0)
    int windowSize;
    int n_s; // N(s)
    int n_r; // N(r)
...
void TransportProtocol::handleMessage(cMessage *msg)
{
   ...
}
