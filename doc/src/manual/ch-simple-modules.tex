...
If the method sets the \ttt{deleteMessage} flag in the result object, that
...
TODO simplify: read params in-place, and explain that the normal implementations cache param values, and override paramChanged() to track changes
...
why no timers: then it would become a simple module
...
\begin{itemize}
  ...
  \item{bit error rate (errors/bit)\index{channel!ber}}
...
The {\opp} class library provides functions to check whether a certain
output gate is transmitting and find out when when it finishes transmission.
