...
signals whether the signal has listeners, so \ffunc{emit()} can return
...
ancestor chain as high as needed. For other signals, \ffunc{emit()} needs
...
arrange that frequently emitted signals (e.g. \ttt{"tx-begin"}) get the
``fast'' signal IDs, while infrequent signals (like e.g.
\ttt{"router-down"}) get the rest.
...
\ffunc{getSignalTime()} is pure virtual (it must return some value),
but \ffunc{getSignalValue()} has a default implementation that
...
The values in the property will get recorded into the output files as result
attributes, and will be available for result processing.

\textbf{Result attributes.} The following result attributes are understood:
...
\begin{description}
  ...
  \item[interpolationMode]: Possible values are "none", "sample-hold"; "backward-sample-hold"; "linear"
  \item[type]: XXX int, double, enum
  \item[enum]: XXX "IDLE=1,BUSY=2,DOWN=3"
  \item[enumName]: xxx
  \item[modeHint]: xxx
\end{description}

Currently the following values are supported  for \ttt{modeHint}:
...
\begin{description}
  ...
  \item[timeavg]: record the time average, interpreting the signal as step function (sample-hold mode)
  \item[stddev]: record the standard deviation of the values
  \item[histogram]: record a histogram of the values (bins are set up automatically)
\end{description}
