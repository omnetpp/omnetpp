...
and the next gate in the path, so a path can be thought of as a double-linked
list. The two pointers are returned by the \fname{getToGate()} and
\fname{getFromGate()} methods, where \textit{to} and \textit{from}
refer to the direction of the arrow, as illustrated on figure
...
The start and end gates of the path can be found with the \fname{getSourceGate()}
and \fname{getDestinationGate()} methods, which simply follow the \textit{from} /
\textit{to} pointers until they return \ttt{NULL}.
...
cGate *otherGate = gate->getType()==cGate::OUTPUT ? gate->getToGate() :
                                                    gate->getFromGate();
if (otherGate)
