...
The value in a \cclass{cPar} object can be read with methods that correspond
to the parameter's NED type: \ffunc{boolValue()}, \ffunc{intValue()},
\ffunc{doubleValue()}, \ffunc{stringValue()}/\ffunc{stdstringValue()},
\ffunc{objectValue()}, \ffunc{xmlValue()}.
There are also overloaded type cast operators for the corresponding types
(\ttt{bool}, integer types including \ttt{int} and \ttt{long},
\ttt{double}, \ttt{const char *}, \ttt{cObject*} and \ttt{cXMLElement*}).
...
Volatile parameters are often used for allowing stochastic input, things like
random packet generation interval specified e.g. as \ttt{exponential(1.0)}
...
parameter is assigned an expression like \ttt{exponential(1.0)}, multiple
...
Object parameters allow for JSON-style parameters and many interesting use cases.
These use cases, along with real-life examples, were presented in the NED chapter,
section \ref{sec:ned-lang:object-parameters}.
...
static method. The enum values are \ttt{BOOL}, \ttt{DOUBLE}, \ttt{INT},
\ttt{STRING}, \ttt{OBJECT}, and \ttt{XML}; and since the enum is an inner type,
...
\begin{note}
  As a rule of thumb, when implementing a \ffunc{handleParameterChange()}
  method, care must be taken that it is prepared to handle the changes of all
  parameters that are marked as \fprop{@mutable} in the NED file.
\end{note}
...
\index{message!canceling} (i.e. removed from the FES\index{FES}).
...
simulation time, it first needs to be canceled using \ffunc{cancelEvent()}.
...
gate\index{output!gate} using one of the overloaded \ffunc{send()} methods
of \cclass{cSimpleModule}. Since the gate can be specified in multiple ways
and the methods also accept an optional \cclass{SendOptions} structure,
...
argument). If the name identifies a gate vector, an additional \ttt{gateIndex}
...
Using a gate pointer (\ttt{cGate*}) will result in more efficient code, since it
spares the lookup inside the \ffunc{send()} call. The module code would typically
...
Using a gate ID (\ttt{gateId} argument) is only a little less efficient than the
...
The \ffunc{send()} call will cause the message to travel along the full length
...
\cclass{cGate}; the path ends where \ffunc{getNextGate()} returns \ttt{nullptr}.
...
as argument (and with some extra argument such as \cclass{SendOptions}).
...
where \ffunc{getNextGate()} returns \ttt{nullptr}), the message will be given to
the \ffunc{arrived()} method of the module to which the last gate belongs. The
\ffunc{arrived()} method normally, i.e. in its default \cclass{cSimpleModule}
...
given to the module's \ffunc{handleMessage()} (or \ffunc{activity()}) method
...
However, it is noteworthy that the \ffunc{arrived()} implementation of
...
\ffunc{send()} calls -- what you have to do instead is
...
input gate of another module, which is useful for scenarios such as simulating
wireless transmissions. \ffunc{sendDirect()} has several variants, because the
target gate can be specified in various ways, a propagation delay and duration
can be optionally given, and the latter two can also be specified using a
...
sent via \ffunc{sendDirect()}. It is not possible to have a gate which receives
...
struct that has the \ttt{updateTx} field set to \ttt{true}.

Transmission updates are paired with the original packet they modify using a
unique transmission ID. The \cclass{SendOptions} methods that set
\ttt{updateTx=true}, \ffunc{finishTx()} and \ffunc{updateTx()}, also require a
transmission ID to be specified.

To allow a packet transmission to be updated later, a transmission ID needs to
be chosen (it is often the ID of the original packet), and remembered for later.
...
Packets in OMNeT++ are delivered to modules similarly to normal messages,
through the \texttt{handleMessage()} method. When a module expects packets to
arrive via a transmission channel, it is necessary to cast the incoming message
to \cclass{cPacket} to access packet-specific methods and properties.

\cclass{cPacket} has several fields that carry information about the packet's
last transmission over the transmission channel. These fields are:

\begin{itemize}
  \item \ffunc{hasBitError()}: Determines if the packet has any bit errors.
  \item \ffunc{isUpdate()}: Checks if the packet is a transmission update.
  \item \ffunc{getTransmissionId()}: For transmission updates, returns the ID for identifying the transmission.
  \item \ffunc{isReceptionStart()}: Checks if the packet's arrival time represents the beginning of the reception of the packet.
  \item \ffunc{isReceptionEnd()}: Checks if the packet's arrival time represents the end of the reception of the packet.
...
Based on the information carried by those fields, processing of the received
packet typically involves handling the following:

\begin{itemize}
    \item \textbf{Bit Errors}: Use the \texttt{hasBitError()} method to check if the packet has arrived with errors.
    \item \textbf{Reception Timing}: Control whether packets are delivered at
      the start or end of their reception using \texttt{setDeliverImmediately()}.
      Utilize \texttt{isReceptionStart()} and \texttt{isReceptionEnd()} to
      determine the reception phase of the packet.
    \item \textbf{Transmission Updates}: Recognize transmission updates using \texttt{isUpdate()}, and react to them accordingly.
\end{itemize}
...
By default, packets are delivered at the end of their reception. To alter this
behavior, use \texttt{gate("in")->setDeliverImmediately(true);} in the module's
\texttt{initialize()} method. This setting makes packets available at the
reception start.
...
\ffunc{setDeliverImmediately()} only needs to be called once,
...
\ttt{setDeliverImmediately(true)}, see previous section) should be prepared to receive
all of the original packet and the updates, and handle them appropriately.
Transmission updates can be recognized by the packet's \ffunc{isUpdate()} method
returning \ttt{true}. cPacket::getRemainingDuration() returns the remaining
transmission duration, and cPacket::getDuration() the total transmission
duration.

As a safeguard against unprepared modules accidentally processing transmission
updates as independent packets, the module is only given transmission updates if
it explicitly declares that it is able to handle them properly. The latter is
done by the module calling \ttt{setTxUpdateSupport(true)}, usually in
\ffunc{initialize()}.
...
The \ttt{demux} result filter in {\omnetpp} provides a mechanism for
...
The \ttt{demux} filter operates by demultiplexing its input into several
...
With the \ttt{demux} filter in operation, the generated statistics names
