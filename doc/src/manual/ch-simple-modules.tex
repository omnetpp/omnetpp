...
\ttt{simtime\_t} is a typedef to \ttt{SimTime} class, which stores simulation
...
\begin{note}
   Earlier \opp used \ttt{double} for simulation time, but this was less than
   ideal. In floating point, large values can only be represented with less
   precision, and this caused problems in long simulations. Non-associativity
   (often $(x+y)+z \neq x+(y+z)$, see~\cite{Goldberg91what}) and accumulating
   rounding errors meant that two \ttt{double} simulation times could not be
   compared for equality.
   
   \opp can still be compiled with \ttt{simtime\_t} defined as \ttt{double},
