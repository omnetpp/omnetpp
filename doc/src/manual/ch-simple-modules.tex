...
messages will be created as \ttt{new cMessage("foo")} where
...
% rewrite to handleMessage, or just remove
...
gate of another module. The \ffunc{sendDirect()} function does that:
...
In addition to the message and delay arguments, it also takes the
...
It is also possible to send to an input gate of a \textit{compound} module.
In that case, the message will follow the connection path inside the
compound module, and will be delived to the module at the end of the path
...
\ttt{sendDirect()}. It is recommended to tag the gate with the
\ttt{@directIn} property in the module's NED declaration; this causes
...
Note that the above code invoked \ttt{getTransmissionFinishTime()} on the
...
\ref{sec:ch-simple-modules:model-change} describes the mechanism to get
...
as well; there, the duration is an argument to the sendDirect() call.
