...
event simulation (DES) works, in order to introduce terms.
...
\begin{itemize}
  \item \ffunc{initialize()}. This method gets invoked after {\opp} has
  ...
  \item \ffunc{finish()} gets called when the simulation has terminated
...
The \ffunc{initialize()} and \ffunc{finish()} method are declared
...
avaiable. In contrast, \ffunc{initialize()} gets called just before the
...
The calling order is the reverse of the order of \ffunc{initialize()}: first
...
To get notified, redefine the \ffunc{handleParameterChange()} method
...
\begin{note}
  ...
  \ffunc{initialize()} / \ffunc{finish()} and \ffunc{handleParameterChange()}
...
you find a channel whose \ffunc{isTransmissionChannel()} method returns
\ttt{true}, but there is a convenience method for this, named
\ffunc{getTransmissionChannel()}. An example:
...
To spare looking up the gate by name for each send operation,
we can use gate IDs and exploit the fact that the ID of gate
...
followed by a \ffunc{send()} because it spares one event. The second, less
...
Note that the above code invokes \ttt{getTransmissionFinishTime()} on the
output gate, not on the channel. \cclass{cGate}'s
\ffunc{getTransmissionFinishTime()} and \ffunc{isBusy()} methods are just
shortcuts to \ttt{getDatarate\-Channel()->isBusy()} and
\ttt{getDatarate\-Channel()->\-getTrans\-mission\-FinishTime()}, provided for
convenience.

The \ttt{getDatarateChannel()} method searches the connection path each
time it is called, so the performance of the gate's
\ttt{getTransmissionFinishTime()} method is not as optimal as it could be.
If performance is important, it is recommended that you obtain the
transmission channel pointer once, and then call \ffunc{isBusy()} and
\ffunc{getTransmissionFinishTime()} on the channel. When the network topology
...
channel's parent is the compound module.
...
\begin{itemize}
  ...
  \item \ffunc[process()]{void process(cMessage *msg, simtime\_t t, result\_t\& result)}
\end{itemize}

The first two functions are usually one-liners; the channel behavior is
encapsulated in the third function, \ffunc{process()}.

\subsubsection{Transmission Channels}

The first function, \ffunc{isTransmissionChannel()}, should tell whether
the channel is a \textit{transmission channel}, i.e. one that models
transmission duration. Transmission channels sets the duration field of
packets sent through them (see the \ffunc{setDuration()} field of
\cclass{cPacket}).
...
value may be any simulation time which is less than or equals the current
simulation time.

\subsubsection{The process() Function}

The third function, \ffunc{process()} encapsulates the channel's
...
What happens inside the \ffunc{send()} call is this. {\opp} follows the
...
Now, back to the \ffunc{process()} method.
...
Plus it has a constructor that initializes all fields to zero; it is left
out for brevity.
...
messages. If the transmission duration is nonzero, it is the method's
responsibility to call \ttt{setDuration(duration)} on the packet. As a
result of bit error modeling, the method may set the packet's bit error
flag.

If the method sets the \ttt{discard} flag in the result object, that
means that the message object should be deleted by {\opp}; this
...
XXX cGate: getTransmissionFinishTime() just delegates to the similar
method of cChannel, so caution: if there's a delay before the transmission
channel in the path, the delay should be manually deduced! or better,
avoid delay channels in front of a transmission channel in a path.

\subsection{Channel Examples}

To illustrate writing channels, we look at how the built-in channel
...
body of its \ffunc{process()} method is empty:
...
disabled; in disabled state, messages sent though it will be discarded.
This class still models zero transmission duration, so its
\ffunc{isTransmissionChannel()} and \ffunc{getTransmissionFinishTime()}
methods still return \ttt{false} and $0s$. The \ffunc{process()} method
...
\cclass{cDatarateChannel}'s \ffunc{process()} method makes use of
the \ttt{isDisabled}, \ttt{datarate}, \ttt{ber} and \ttt{per} data
members, which are also kept up to date with the help of
\ffunc{handleParameterChange()}. It ensures that no packet gets
transmitted while a previous one is still being transmitted.
It also sets the packet's \textit{duration} field, making sure
that it has not been set before.

FIXME XXX TODO ez igy egy eleg szar design. cGate-nek kellene lecsekkelnie, hogy ez a channel transmissionchannel-e, es minden boilerplate-et megcsinalni sajat maga. (pl packet->setDuration()-t is!)
...
void cDatarateChannel::process(cMessage *msg, simtime_t t, result_t& result)
{
    ...
    if (txfinishtime > t)
        throw cRuntimeError(this, "Channel is currently busy...");

    // datarate modeling
    if (datarate!=0 && msg->isPacket()) {
        cPacket *pkt = (cPacket *)msg;
        if (pkt->getDuration() != 0)
            throw cRuntimeError(this, "Packet already has a duration set...");
...
dynamically deleted before that, \ffunc{finish()} will not get invoked
