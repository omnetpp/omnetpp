...
%---------------
%TBD
%Point: floating point errors shouldn't matter. Simulation is the model
%of the real world. When two events in the real world occur "simultaneously",
%their "order" usually cannot be determined and is basically unpredictable,
%so it should not matter in which order we simulate them.
%
%Try increments of 0.1 -- errors will soon come in
%
%As an aside: One could think about switching "simtime_t" to "long double".
%This doesn't seem to be a very good idea. While "double" is standard 64-bit
%IEEE-754 practically everywhere, there doesn't seem to be an agreement what
%"long double" should mean in C/C++: 64, 80, 96 or 128 bits (see e.g. [1])
%-- so simulations could yield somewhat different results on different
%platforms, which is not very desirable.
%For example, in Win32, "long double"=="double".
%
% User comment:
%> So why not use some integer based time instead? If you are using a
%> 64bit integer value, and have some functions to setup the timebase
%> before the simulation run (how much a difference of 1 in the integers
%> represent in the timebase, for instance 1ns or 1ps) you could get rid
%> of all the nasty effects of floating-points with the drawback of some
%> runtime limitations. But still if use a 64bit integer with an timebase
%> resolution of 1ns (nano-second), you can represent a timespace about
%> 584 years, which should be quiet enough for the most simulations.
%> Usually integer operations are still faster than floating-point ones
%> (especially doubles) and so simulation speed can be enhanced as well.
%
%FIXME TBD
% Re: integer-based simulation time
%
%> It's a good point that 64-bit integers would provide enough range and
%> resolution (585yrs with ns prec.) for most simulations. But certainly
%> for not all (some special studies may require sub-ns times, or longer
%> time than 585yrs), and the other drawback is that C++
%> provides no comfortable way of working with such numbers (there isn't
%> such 1e9-shifted fixed-point mode in C/C++ so you'd have to calculate
%> *everything* in nanoseconds, which is quite a nuisance.) Practically,
%> to switch the sim kernel to 64-bit integer time (long long, int64,
%> __int64 on msvc) one would have to change the simtime_t typedef and
%> the simtimeToStr()/ strToSimtime() funcions at the bare minimum, but
%> probably there'be other places to change as well (printf's "%lg" is
%> probably used at some places etc). I don't think it's really feasible.
%>
%> But even without all that, when you have beautiful 52-bit integer arithmetic
%> already built in! I'm talking about the IEEE double-prec number's
%> 52-bit mantissa.
%> If you stick to putting integers into doubles, that'll give you
%> *precise* calculations up to 2^52 = 4.5e15, that is 1250hrs with ns
%> precision. To do that you'd go on with the current sim kernel
%> unchanged, but interpret simtime_t in nanoseconds not seconds, and you
%> restrict yourself to use only integer values with it. The time
%> displayed on the UI which comes from
%> simtimeToStr()/strToSimtime() would be crap of course (unless you
%> replace those functions), but all time calculations will be precise.
%>
%> If you'd like to experimentally verify this, please compile and run
%> the attached little program.
%
%#include <stdio.h>
%#include <math.h>
%
...
%{
%    // change these two
%    double start = pow(2.0,50.0);
%    double increment = 99.0;
%
%    double x = floor(start);
%    int i = 0;
%    for (;;)
%    {
%        double xprev = x;
%        x = x + increment;
%        if (floor(x)!=x) printf("\nProblem at x=%lg: not whole number\n", x);
%        if (x-xprev != increment) printf("\nProblem at x=%lg: increment not right\n", x);
%        if ((i++ & 0x00ffffff)==0) printf("."); // monitor progress
%    }
%}
%---------------
...
If a message travels along a route, passing through successive links and
...
message are calculated immediately after the \fname{send()} (or a similar)
function is invoked. That is, if the message travels through several
links before it reaches its destination, it is \textit{not} scheduled
...
\begin{itemize}
  \item{write a sender module such that it schedules events for when the
...
but where would it say Hello then? Note \cclass{cSimpleModule} as base class,
...
in NED files, we should have an associated NED declaration which might
look like this:
...
\begin{enumerate}
  ...
  \item{\ttt{cSimpleModule(size_t stacksize)} -- one that accepts the coroutine stack size}
\end{enumerate}
...
compiler-generated one is also suitable.
...
{\opp} versions earlier than 3.2 expected a different module class
...
For convenience, a macro named \fmac{Module\_Class\_Members()} was also
provided, which expanded to a default (i.e. do-nothing) constructor implementation.

In {\opp} 3.2, the \fmac{Module\_Class\_Members()} macro has been retained
but expands to the new constructor definition. Thus a module which uses
\fmac{Module\_Class\_Members()} does not need to be changed to work with
{\opp} 3.2 or later. When compatiblity with older versions is no longer
required, the macro call can simply be deleted.

Some (few) modules had hand-coded constructors instead of using
\fmac{Module\_Class\_Members()}. These modules will produce a compile
error with {\opp} 3.2 or later, saying \textit{no appropriate constructor
available}. The easiest way to get them working is to add \ttt{=NULL}
default value to both the \ttt{name} and the \ttt{parentModule} arguments:

\begin{verbatim}
MyModule(const char *name=NULL, cModule *parentModule=NULL, size_t stack=<stacksize>);
\end{verbatim}

Again, when compatibility with older {\opp} versions is no longer required,
...
with {\opp} internals or lacking advanced C++ skills.
    \footnote{These crashes occurred due to lack of information available
              ...
              if an object is part of an array, or it's inside a struct or class.}
...
It is expected that existing modules will be updated for {\opp} 3.2 sooner
or later, by adding proper constructors and destructors. To catalyse this
process, {\opp} dumps the list of unreleased objects at the end of the
simulation. This dump can also be turned off in the configuration
(\ttt{print-undisposed=} configuration option, see \ref{sec:ch-run-sim:general-section}).
...
so the \fname{parentModule()} and \fname{submodule()} methods of
