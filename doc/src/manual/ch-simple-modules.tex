...
The \ttt{SimTime} class performs additions and subtractions as 64-bit integer
...
with simulation visualization (\ref{sec:ch-graphics:refreshdisplay}).
...
code: if \ttt{calleeModule} from the first line is \ttt{nullptr} because
...
they has to know about them. Third, method calls are also recorded
in the event log.
...
also an (optional) argument of \ffunc{emit()}.
...
\begin{note}
  The \textit{details} parameter was added in {\opp} 5.0.
\end{note}
...
\ref{sec:ch-ned-lang:properties}). The property index corresponds
...
model change notifications (see \ref{sec:ch-simple-modules:model-change}).
...
class cIListener
{
  public:
    ...
    virtual void receiveSignal(cComponent *src, simsignal_t id,
                               bool b, cObject *details) = 0;
    virtual void receiveSignal(cComponent *src, simsignal_t id,
                               long l, cObject *details) = 0;
    virtual void receiveSignal(cComponent *src, simsignal_t id,
                               double d, cObject *details) = 0;
    virtual void receiveSignal(cComponent *src, simsignal_t id,
                               simtime_t t, cObject *details) = 0;
    virtual void receiveSignal(cComponent *src, simsignal_t id,
                               const char *s, cObject *details) = 0;
    virtual void receiveSignal(cComponent *src, simsignal_t id,
                               cObject *obj, cObject *details) = 0;
...
The latter can be achieved by calling \ffunc{ev.addResultRecorders()}.
...
As an alternative to \fprop{@statisticTemplate} and \ffunc{ev.addResultRecorders()},
