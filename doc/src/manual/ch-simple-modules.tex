...
Parameters declared in the NED file can be marked as \texttt{volatile}. When a
parameter is tagged as \texttt{volatile}, it indicates that reading the
parameter's value during simulation may yield a different value each time.
(Reading the value will cause a re-evaluation of the NED expression, which may
contain e.g. calls to the random number generator.) Consequently, within the
model code, it is essential to re-fetch the parameter's value each time it is
required during simulation. In other words, simply reading the parameter once in
the \texttt{initialize()} function and storing that value for subsequent use
is incorrect.
...
The method \ttt{objectValue()} is used to access object parameters. It returns a
pointer of the type \ttt{cObject*}, which must be cast to the appropriate type
using \ttt{check\_and\_cast()} or \ttt{dynamic\_cast()}. Object parameters are
...
with arrays represented by the \cclass{cValueArray} class and dictionaries
represented by the \cclass{cValueMap} class. Values inside \cclass{cValueArray}
and \cclass{cValueMap} are represented with the \cclass{cValue} class. When a
single value is assigned to an object parameter, it is represented as a
\cclass{cValue} wrapped in a \cclass{cValueHolder}. \footnote{Object parameters
...
\cclass{cStringTokenizer} class and its convenience methods, such as
\ffunc{asIntVector()} and \ffunc{asDoubleVector()}.}
...
with the \ttt{\$i} and \ttt{\$o} name suffixes. Thus, the gate name needs to be
