...
If the parameter was declared \textit{volatile} in the NED file
or its value is expected to change at runtime (for example by other modules),
the code should to store a reference or a pointer to the \cclass{cPar} object,
and re-read the value each time it is needed.
...
{
    //...
    wait((double)waitTime);
}
\end{verbatim}

If the \ttt{waitTime} parameter is volatile and it was given a random value
(e.g. \ttt{exponential(1.0)}) in the NED source or the ini file, the above
code results in a different delay each time.

Parameter values can also be changed from the program, during
execution.

The \cclass{cPar} class is discussed in more detail in the next section.


\subsection{The cPar module parameter class}

Module parameters are represented as \cclass{cPar} objects.
The module parameter name is the \cclass{cPar} object's name (\fname{getName()}),
and the object can store any parameter type supported by the NED language:
long, double, bool, string and XML config file reference.

%    \footnote{\cclass{cPar} can no longer be used for message parameters;
%    a separate class, \cclass{cMsgPar} has been created for that purpose.}
%
%
%    Type getType() const;
%
...
\cclass{cMsgPar} several methods for setting and accessing the value:
\fname{setBoolValue()}, \fname{setLongValue()}, \fname{setStringValue()},
\fname{setDoubleValue()}, \fname{setXMLValue()}; and
\fname{boolValue()}, \fname{longValue()}, \fname{stringValue()},
\fname{doubleValue()}, \fname{xmlValue()}.
There are also overloaded type cast and assignment operators for C/C++
primitive types including \ttt{bool}, \ttt{int}, \ttt{long}, \ttt{double},
\ttt{const char *}, and also for \ttt{cXMLElement *}.
...
&
Reference to an XML element in an XML config file.
\\\hline
\end{longtable}
