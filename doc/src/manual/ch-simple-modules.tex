...
The simulation stops when there are no more events left (this happens
...
\begin{itemize}
  \item{declaration of the module class: your class directly or indirectly
        subclassed from \cclass{cSimpleModule}}
  \item{a module type registration (\fmac{Define\_Module()} macro)}
      \footnote{For completeness, there is also a \fmac{Define\_Module\_Like()}
                macro, but it's rarely useful.}
...
As an alternative to \fmac{Define\_Module()}, one can the use
\fmac{Define\_Module\_Like()} macro to associate a class with a NED
simple module declaration of a \textit{different} name. An example:
...
This would associate the \ttt{TokenRingMAC}, \ttt{EthernetMAC} and
\ttt{FDDIMAC} C++ classes (which don't have NED counterparts)
with the \ttt{GenericMAC} module defined in NED (only). Since
\ttt{TokenRingMAC}, \ttt{EthernetMAC} and \ttt{FDDIMAC} don't appear
in NED files, they cannot be used directly, only via NED's \ttt{like}
construct \ref{sec:ch-ned-lang:like}.

Although \fmac{Define\_Module\_Like()} may occasionally spare you
writing some NED files, its use is not recommended, mainly because
modules defined with it don't show up in documentation generated
with \ttt{opp_neddoc} \ref{cha:neddoc}.
...
arguments (which also have to be passed further to the base class):
...
functions issue error messages telling you what the problem is.
...
Cleanup code should go into the destructor\index{module!destructor}.
Actually, you rarely need to write a destructor yourself, because {\opp}
keeps track of objects you create and disposes of them automatically
(a sort of automatic garbage collection). It cannot track
objects that are not derived from \cclass{cObject}, however, so they may
need to be deleted manually by the destructor. Garbage collection
