...
The user creates simple\index{module!simple} module types are by
...
[cooperative] threads), and \fname{handleMessage()} is a function
...
\fvar{simtime\_t}, which is a typedef for double.\\
...
\item{a constructor\index{module!constructor} with the argument list: (const
    ...
    \textit{stacksize})}
\item{a \fname{className()} function which returns the name of the
    class as char*}
\end{itemize}
...
code. Also, remember to overwrite the \fname{className()} function.
