...
Packets in {\omnetpp} are delivered to modules similarly to normal messages,
...
\begin{itemize}
  ...
  \item \ffunc{isReceptionStart()}: Returns \ttt{true} if the packet's delivery to the module represents the beginning of the reception of the packet.
  \item \ffunc{isReceptionEnd()}: Returns \ttt{true} if the packet's delivery to the module represents the end of the reception of the packet.
...
packet typically involves handling the steps described in the following sections.
...
\ffunc{isReceptionStart()} and \ffunc{isReceptionEnd()} methods can be called to
determine whether it corresponds to the start or end of the reception process.
...
The receiver should recognize transmission updates using \texttt{isUpdate()}, and
...
\ttt{setDeliverImmediately(true)}, see previous section) should be prepared to
receive all of the original packet and the updates, and handle them
appropriately. If an update arrives, the receiver should replace the original
packet with the update, and reschedule any potential end-reception event to
\ttt{simTime() + pk->getRemainingDuration()}.
...
Non-transmission channels handle updates in the same way as any other messages
and packets.
