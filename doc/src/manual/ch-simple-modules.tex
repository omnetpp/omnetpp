...
(\ttt{perform-gc=} configuration option, see \ref{sec:ch-config-sim:general-section}),
...
the unit of measurement associated with the parameter (\ttt{@unit} property in NED).
...
deal with gates. You can look up a gate by name using the \fname{gate()}
method:
...
error. The \fname{gate()} method needs to be told whether the input or
...
Another way is to use the \fname{gateHalf()} function, which takes
...
The \fname{hasGate()} method can be used then. An example:
...
You can get the ID from the gate itself (\fname{getId()} method),
or from the module by gate name (\fname{findGate()} method).
The \fname{gate()} method also has an overloaded variant which
...
the \fname{gate()} function with an additional \textit{index}
parameter. The index should be between zero and \textit{size}-1.
The size of the gate vector can be read with the \fname{gateSize()}
...
A gate vector cannot have ``holes'' in it, that is, \fname{gate()}
never returns \ttt{NULL} or throws an error if the gate vector exists
and the index is within bounds.

For inout gates, \fname{gateSize()} may be called with or without
the \ttt{"\$i"}/\ttt{"\$o"} suffix, and returns the same number.

The \fname{hasGate()} method may be used both with and without an
...
but it is better to use a dedicated method, \fname{gateBaseId()},
...
two possibilities. One is invoking the \fname{getGateNames()} method
that returns the names of all gates and gate vectors the module
has; then you can call \fname{isGateVector(name)} to determine
...
gates \fname{getGateNames()} returns the base name without the
\ttt{"\$i"}/\ttt{"\$o"} suffix, so the two directions
need to be handled separately. The \fname{gateType(name)} method
...
\begin{note}
    ...
    Moreover, the \fname{gate()} method now throws an error when called
...
\fname{addGate(name,type,isvector=false)} and \fname{deleteGate(name)}.
Gate vector size can be changed by using \fname{setGateSize(name,size)}.
...
The \fname{getName()} method of \cclass{cGate} returns the name of the
gate or gate vector. If you need a string that contains the gate index
as well, \fname{getFullName()} is what you want. If you also want to
include the hierarchical name of the owner module, call \fname{getFullPath()}.

The \fname{getType()} method of \cclass{cGate} returns the gate type,
either \ttt{cGate::INPUT} or \ttt{cGate::OUTPUT}. (It cannot return
\ttt{cGate::INOUT}, because an inout gate is represented by a pair
of \cclass{cGate}s.) The \fname{isVector()}, \fname{getIndex()},
\fname{getVectorSize()}, \fname{getId()} method names speak for themselves.
\fname{size()} is an alias to \fname{getVectorSize()}.

The \fname{getOwnerModule()} method returns the module the gate object
belongs to.
...
and the next gate in the path (returned by the \fname{getPreviousGate()}
and \fname{getNextGate()} methods), so a path can be thought of as
...
The start and end gates of the path can be found with the \fname{getPathStartGate()}
and \fname{getPathEndGate()} methods, which simply follow the \textit{previous gate} /
\textit{next gate} pointers until they return \ttt{NULL}.

The \fname{isConnectedOutside()} and \fname{isConnectedInside()} methods
...
The \fname{isConnected()} method is a bit different: it returns true if the gate
...
You can achieve the same effect by calling the \fname{error()} method of
\cclass{cModule}.
...
Of course, the \fname{error()} method can only be used when a module pointer
is available.
...
\cclass{cGate}'s \fname{getPreviousGate()}, \fname{getNextGate()} and
...
For input gates, you would use \fname{getPreviousGate()} instead of
\fname{getNextGate()}.
