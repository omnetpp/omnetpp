...
\ffunc{getTransmissionFinishTime()} and \ffunc{isBusy()} methods are
shortcuts to \ttt{getDatarate\-Channel()->isBusy()} and
\ttt{getDatarate\-Channel()->\-getTrans\-mission\-FinishTime()}, provided for
convenience.

\begin{note}
  \cclass{cGate}'s \ffunc{getTransmissionFinishTime()} function just
  delegates to the same method of the transmission channel in the path, so
...
messages. \ffunc{process()} does not need to call the \ffunc{setDuration()}
...
The \ffunc{process()} method does not need to throw error on overlapping
transmissions, or if the packet's duration field is already set; these
checks are done by the simulation kernel before \ffunc{process()} is called.
