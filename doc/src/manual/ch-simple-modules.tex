...
\begin{itemize}
  \item Several overloaded \ffunc{receiveSignal()} methods, one for each
    ...
    are invoked.
  \item \ffunc{finish()}: Method gets called when XXX FIXME TODO WHICH COMPONENT's
    \ffunc{finish} method gets called. This method can be used to record
  ...
  \item \ffunc{listenerAdded()}, \ffunc{listenerRemoved()}. They are called
    ...
    The \ffunc{listenerRemoved()} method is also called from \cclass{cComponent}'s
...
\begin{cpp}
simulation.getSystemModule()->subscribe(PRE\_MODEL\_CHANGE, listener);
\end{cpp}
