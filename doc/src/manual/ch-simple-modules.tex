...
Since {\opp} 4.4, modules (and channels) can declare the signals they emit
in their NED definitions, and {\opp} can be configured to enforce that
emitted signals conform to the declarations.

Signals are declared with the \fprop{@signal} property (NED properties are
described in section \ref{sec:ch-ned-lang:properties}). The signal name is
written in the property index, and the property's body may declare various
attributes of the signal (currently only the data type is supported).

The following example declares a queue module that emits a queue length signal:
...
\ttt{type} is optional; when present, its value should be \ttt{bool},
\ttt{long}, \ttt{unsigned long}, \ttt{double}, \ttt{simtime\_t},
...
\ffunc{isInstance()} just wraps a C++ \ttt{dynamic\_cast}.) A question mark
after the class name means that the signal is allowed to emit \ttt{NULL}
pointers, too.

For example, a module named \ttt{PPP} may emit the frame (packet) object
every time it starts transmiting, and emit \ttt{NULL} when the transmission
is completed:
...
Currently, signal checking is turned off by default, you can turn it
...
It is expected that starting with {\opp} 5.0, signal checking will be turned
on by default when the simulation kernel is compiled in debug mode.
