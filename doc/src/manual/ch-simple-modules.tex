...
directly. If you're deleting a compound module, \ffunc{callFinish()} will
recursively invoke \ffunc{finish()} for all submodules, and if you're deleting
...
These goals are achieved with the following implementation. First, the data
structure that used to store listeners in components is
dynamically allocated, so if there are no listeners, the per-component
overhead is only the size of the pointer (which will be \ttt{NULL} then).
...
typically less than 64 signals used at a time in a simulation.}
Using these bitfields, it is possible to determine in constant time
for the first 64 signals whether the signal has listeners, so \ffunc{emit()} can
return immediately if there are no listeners, does not need to go up
the ancestor chain of there only local listeners, and otherwise only needs
to go up the ancestor chain as high as needed.
...
The \cclass{cListener} class maintains a reference count (increments it in
every \ffunc{listenerAdded()} call and decrements it in
\ffunc{listenerRemoved()}). When a \cclass{cListener} object is deleted,
its destructor checks that the reference count is zero, and if not, it
informs the user with an error message. This is one more reason to
use \cclass{cListener} as a base class for your listeners.
