...
specifying -u Cmdenv on the command line. If both the config option and the command line option 
...
\begin{itemize}
  ...
  \item \ttt{-x ConfigName} -- Prints the number of runs available in the configuration.
  \item \ttt{-g} -- Prints the unrolled configuration (together with the -x option) and
                    expands the iteration variables.
  \item \ttt{-G} -- Prints even more detail than -g.
\end{itemize}

User interfaces may support the \ttt{-r runnumber} option to select runs, either one or more, 
...
\begin{itemize}
  \item \ttt{cmdenv-express-mode} -- Provides only minimal status updates on the console.
  \item \ttt{cmdenv-interactive} -- Allows asking interactively for missing
        parameter values.
  \item \ttt{cmdenv-status-frequency} -- How often the status is written to the console.
  \item \ttt{cpu-time-limit} -- Limit how long the simulation should run (in wall clock time)
  \item \ttt{sim-time-limit} -- Limit how long the simulation should run (in simulation time)
  \item \ttt{debug-on-errors} -- If the runtime detects any error it will generate a breakpoint
        so you will be able to ckeck the location and the context of the problem in your debugger.
  \item \ttt{fingerprint} -- The simulation kernel computes a checksum while running the simulation.
          It is calculated from the module id and from the current simulation time of each event. 
          If you specify the \ttt{fingerprint} option in the config file, the simulation runtime will
          ...
          f the model.
  \item \ttt{record-eventlog} -- You can turn on the recording of the simulator events. The 
...
\ref{cha:appendix-config-options:config.options} for the list of
...
\ttt{sim-time-limit} configuration option, but you can limit your simulation 
by using wall clock time (\ttt{cpu-time-limit}) or by directly ending a 
