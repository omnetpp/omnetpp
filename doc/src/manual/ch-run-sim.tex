...
is possible to specify only a single user interface with the \fopt{-u
Cmdenv} or \fopt{-u Tkenv} option on the \fprog{opp\_makemake} command line.
...
\ttt{user-interface=Cmdenv} (or \fopt{=Tkenv}) option in your ini file, or
by specifying \fopt{-u Cmdenv} or \fopt{-u Tkenv} on the command line. If
both the config option and the command line option are present, the command
line option takes precedence.

\subsubsection{Selecting Configuration and Runs}
\label{sec:run-sim:selecting-configuration-and-run-number}

Configurations can be selected with the \fopt{-c <configname>} command line option.
...
User interfaces may support the \fopt{-r <runnumber>} option to select runs,
...
\begin{itemize}
  \item \fopt{-a} -- Prints all configuration names and the number of runs in them.
  \item \fopt{-x <configname>} -- Prints the number of runs available in the given configuration.
  \item \fopt{-g} -- Prints the unrolled configuration (together with the \fopt{-x} option) and
                    expands the iteration variables.
  \item \fopt{-G} -- Prints even more details than \fopt{-g}.
\end{itemize}
...
Use the \fopt{-l <libraryname>} command line option to load a library dynamically.
...
\begin{itemize}
  ...
  \item \fconfig{cpu-time-limit} -- Limits how long the simulation should run (in real time)
  \item \fconfig{simtime-resolution} -- Defines the resolution of simulation time. Acceptable
          values are SI time units (\ttt{s}, \ttt{ms}, \ttt{us}, \ttt{ns}, \ttt{ps}, \ttt{fs},
          \ttt{as}), power-of-ten multiples of such units (e.g. \ttt{100ms}), and base-10 scale
...
using the \fopt{-r 2,4,6..8} format. See \ref{sec:run-sim:batch-execution}
...
The \fopt{-r} command line option overrides this ini file setting.
...
options. \index{command line options} Additionally, the \fopt{-c <configname>}
and \fopt{-r <runnumber>} options can be used to preselect a single run for execution;
...
\begin{itemize}
  \item \fconfig{tkenv-default-config}:
    ...
    default is to ask the user. This option is equivalent to the \fopt{-c}
    command-line option.
...
by prefixing them with two dashes (e.g \fopt{-{}-tkenv-extra-stack=32KiB}). See
...
Qtenv\index{Qtenv} is a runtime simulation GUI, intended to replace Tkenv
...
default runtime. Qtenv can be activated by adding the \fopt{-u Qtenv} option
to simulation command lines. Alternatively, one can configure and build
{\opp} so that Qtenv is the default GUI. (Hint: specify \ttt{PREFER\_QTENV=yes}
in \ffilename{configure.user}; see the \textit{Installation Guide} for details.)
...
by prefixing them with two dashes (e.g \fopt{-{}-qtenv-extra-stack=32KiB}). See
Appendix \ref{cha:config-options} for the list of possible configuration options.
...
To execute more than one run using Cmdenv, use the \fopt{-r} option
and specify the runs in a comma separated format \ttt{1,2,4,9..11}, or you may leave
out the \fopt{-r} option to execute all runs in the experiment.
...
\fopt{-x <config>} command line option to print the number of runs or
add the \fopt{-g} to get more details.
...
activate parallel building. The \fopt{-j} option can be used to specify the maximum number
...
\begin{warning}
  ...
  after the \fopt{-j} option, otherwise the \fprog{make} program will try to start \textit{all}
  runs at the same time. As a rule of thumb: if you have 4 cores (and enough memory), use \fopt{-j4}.
\end{warning}
...
You can use the \fopt{-x <configname> -g} command line options with your
simulation to check the number of available runs.

Using the \fopt{-{}-export <filename>} option only generates the
\ttt{makefile}, but does not start it. You can run your batch later by
...
options you may use \fopt{-h config} or \fopt{-h configdetails} options
