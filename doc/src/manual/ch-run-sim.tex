...
option (specify \ttt{Qtenv}, \ttt{Tkenv} or \ttt{Cmdenv} as its argument), or by
...
output. For example, \fopt{-q runs} prints one line of information with 
the iteration variables about each run that the run filter matches.
The list of available format names include \ttt{numruns},
\ttt{runnumbers}, \ttt{runs}, \ttt{rundetails} and \ttt{runconfig}. 
...
The \ttt{-q sectioninheritance} option ignores \fopt{-r}, and prints the
section inheritance chain (the inheritance graph after linearization)
for the configuration denoted by \fopt{-c}. 
...
\begin{itemize}
  ...
  \item \fconfig{fingerprint} : This option is a regressing testing tool that
...
you are leaking other \cclass{cObject}-based objects. You can also
...
in section \ref{sec:run-sim:selecting-configuration-and-runs}.) 

\begin{itemize}
    \item \fconfig{cmdenv-config-name} : Same as the \fopt{-c} option.
    \item \fconfig{cmdenv-runs-to-execute} : Same as the \fopt{-r} option.
    \item \fconfig{cmdenv-stop-batch-on-error} : When performing multiple runs,
...
Tkenv\index{Tkenv} is the traditional Tcl/Tk-based graphical runtime user
...
simply prefix it with \ttt{opp\_runall}:    
