...
with the \ttt{load-libs=} configuration option. The values from the command line
...
\begin{note}
  It is possible to specify a configuration option also on the command line (in which case the
...
\begin{warning}
  Although it is very convenient, we do not recommend thay you use this method for
...
There is no penalty if you set \fpar{total-stack} too high. I
...
this number as the coroutine stack size, and with 1MB being the default,
