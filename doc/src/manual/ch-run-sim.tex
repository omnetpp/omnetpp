...
\begin{itemize}
  ...
  \item \ttt{-g} -- Prints the unrolled configuration (together with the \ttt{-x} option) and
                    expands the iteration variables.
  \item \ttt{-G} -- Prints even more details than \ttt{-g}.
\end{itemize}
...
by prefixing them with two dashes (e.g \ttt{-{}-tkenv-extra-stack=32KiB}). See
...
default runtime. Qtenv can be activated by adding the \ttt{-u Qtenv} option
...
by prefixing them with two dashes (e.g \ttt{-{}-qtenv-extra-stack=32KiB}). See
Appendix \ref{cha:config-options} for the list of possible configuration options.
...
You can use the \ttt{-x <configname> -g} command line options with your
simulation to check the number of available runs.

Using the \ttt{-{}-export <filename>} option only generates the
