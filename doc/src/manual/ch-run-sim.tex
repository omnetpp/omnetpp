...
<configname> is by convention a camel-case
string that starts with a capital letter:
...
Here's an example:
\ttt{\ \ [Config AlohaScenario]}
\ttt{\ \ *.numHosts = \$\{1, 2, 5, 10..50 step 10\}}
\ttt{\ \ **.host[*].generationInterval = exponential( \$\{0.2, 0.4, 0.6\} )}
...
How does it get run? First of all, Cmdenv with the
'-n' option will tell you how many
simulation runs a given section expands to. (You'll of
course use Cmdenv for batch runs, not Tkenv.)
\ttt{\ \ \% aloha -u Cmdenv -c AlohaScenario {--}n}



\ttt{\ \ OMNeT++/OMNEST Discrete Event Simulation}
\ttt{\ \ ...}
\ttt{\ \ Config: AlohaScenario}
\ttt{\ \ Number of runs: 24}
...
for now.
\ttt{\ \ \% aloha -u Cmdenv -c AlohaScenario -n -g}
\ttt{\ \ OMNeT++/OMNEST Discrete Event Simulation}
\ttt{\ \ ...}
\ttt{\ \ Config: AlohaScenario}
\ttt{\ \ Number of runs: 24}
\ttt{\ \ Run 0: \$0=1, \$1=0.2, \$repetition=0}
\ttt{\ \ Run 1: \$0=1, \$1=0.4, \$repetition=0}
\ttt{\ \ Run 2: \$0=1, \$1=0.6, \$repetition=0}
\ttt{\ \ Run 3: \$0=2, \$1=0.2, \$repetition=0}
\ttt{\ \ Run 4: \$0=2, \$1=0.4, \$repetition=0}
\ttt{\ \ Run 5: \$0=2, \$1=0.6, \$repetition=0}
\ttt{\ \ Run 6: \$0=5, \$1=0.2, \$repetition=0}
\ttt{\ \ Run 7: \$0=5, \$1=0.4, \$repetition=0}
\ttt{\ \ ...}
\ttt{\ \ Run 19: \$0=40, \$1=0.4, \$repetition=0}
\ttt{\ \ Run 20: \$0=40, \$1=0.6, \$repetition=0}
\ttt{\ \ Run 21: \$0=50, \$1=0.2, \$repetition=0}
\ttt{\ \ Run 22: \$0=50, \$1=0.4, \$repetition=0}
\ttt{\ \ Run 23: \$0=50, \$1=0.6, \$repetition=0}
...
\ttt{\ \ \% aloha -u Cmdenv -c AlohaScenario -r 0}
\ttt{\ \ \% aloha -u Cmdenv -c AlohaScenario -r 1}
\ttt{\ \ \% aloha -u Cmdenv -c AlohaScenario -r 2}
\ttt{\ \ ...}
\ttt{\ \ \% aloha -u Cmdenv -c AlohaScenario -r 23}
...
\ttt{\ \ \% aloha -u Cmdenv -c AlohaScenario}



\ttt{\ \ OMNeT++/OMNEST Discrete Event Simulation}
\ttt{\ \ Preparing for running configuration AlohaScenario, run \#0...}
\ttt{\ \ ...}
\ttt{\ \ Preparing for running configuration AlohaScenario, run \#1...}
\ttt{\ \ ...}
\ttt{\ \ ...}
\ttt{\ \ Preparing for running configuration AlohaScenario, run \#23...}
...
numbers (unless you want to use the
\textit{"a..b"} or
...
\ttt{\ \ *.greeting = "We will simulate \$\{1,2,5\}
host(s)."}
\ttt{\ \ \ \ \ \ ==> *.greeting = "We will
simulate 1 host(s)."}
\ttt{\ \ \ \ \ \ \ \ \ \ *.greeting = "We will simulate 2
host(s)."}
\ttt{\ \ \ \ \ \ \ \ \ \ *.greeting = "We will simulate 5
host(s)."}
...
\$\{<varname>\}:
\ttt{\ \ [Config Aloha]}
\ttt{\ \ *.numHosts = \$\{N=1, 2, 5, 10..50 step 10\}}
\ttt{\ \ **.host[*].generationInterval = exponential( \$\{mean=0.2, 0.4,
0.6\} )}
\ttt{\ \ **.greeting = "There are \$\{N\} hosts"}
...
\ttt{\ \ *.numNodes = \$\{n=10..100 step 10\}}
\ttt{\ \ **.numNeighbors = \$\{m=2..10 step 2\}}
\ttt{\ \ constraint = \$m <= sqrt(\$n)}
...
\textbf{NOTE}: It is not supported to refer to other iteration variables
...
\textbf{NOTE:} Mersenne Twister, the default RNG of OMNeT++ has a cycle
length of 2\textsuperscript{19937}, which is more than enough for any
conceivable purpose.
...
\$\{runnumber\}:
\ttt{\ \ seed-set = \$\{runnumber\} \ \ \# this is the default}
...
\ttt{\ \ repeat = 4}
\ttt{\ \ seed-1-mt = \$\{53542, 45732, 47853, 33434 ! repetition\}}
\ttt{\ \ seed-2-mt = \$\{75335, 35463, 24674, 56673 ! repetition\}}
\ttt{\ \ seed-3-mt = \$\{34542, 67563, 96433, 23567 ! repetition\}}
...
to the following:
\ttt{\ \ \# no repeat= !}
\ttt{\ \ seed-1-mt = \$\{seed1 = 53542, 45732, 47853, 33434\}}
\ttt{\ \ seed-2-mt = \$\{ \ \ \ \ \ \ \ 75335, 35463, 24674, 56673 !
seed1\}}
\ttt{\ \ seed-3-mt = \$\{ \ \ \ \ \ \ \ 34542, 67563, 96433, 23567 !
seed1\}}
...
\ttt{\ \ output-vector-file =
"\$\{configname\}-\$\{runnumber\}.vec"}
\ttt{\ \ output-scalar-file =
"\$\{configname\}-\$\{runnumber\}.sca"}


This generates file names like
"PureAloha-0.vec",
"PureAloha-1.vec", and so on.
...
\textbf{NOTE}: The old behavior can be turned back on by setting
...
interval were
<module-and-vectorname-pattern>.enabled,
and
<module-and-vectorname-pattern>.interval.
The "enabled" and
"interval" words changed to
"enable-recording" and
"recording-interval".

\textbf{NOTE}: The reason for this change is that per-object
...
by comma. An example:
\ttt{\ \ **.fifo[2].queueLengthVector.enable-recording = false}
\ttt{\ \ **.fifo[*].queueLengthVector.recording-interval = ..100, 300..500,
900..}
...
\ttt{\ \ **.host[*].networkLoad.save-as-scalar = true}
...
\ttt{\ \ **.host[*].startTime = 3}
\ttt{\ \ **.host[*].startTime.save-as-scalar = true \ \# saves "3" once for each host}
...
\begin{verbatim}
\ttt{\ \ **.interarrivalTime = exponential(1)}
\ttt{\ \ **.interarrivalTime.save-as-scalar = true \ \# saves random values!}
\end{verbatim}
...
\ttt{\ \ experiment = "\$\{configname\}"}
\ttt{\ \ measurement = "\$\{iterationvars\}"}
\ttt{\ \ replication = "\#\$\{repetition\},seed-set=<seedset>"}
...
\ttt{\textrm{"PureAloha" }\textrm{\textit{--
experiment}}}
\ttt{\textrm{\$N=1,\$mean=0.2}\textrm{\textit{ -- measurement}}}
\ttt{\textrm{\#0, seed-set=0}\textrm{\textit{ -- replication}}}
...
The tree shows that ("PureAloha",
"\$N=1,\$mean=0.2", "\#0,
seed-set=0") was run three times. The results produced
