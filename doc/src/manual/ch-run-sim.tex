...
The default ini file is \ttt{omnetpp.ini}\index{omnetpp.ini}, and is
...
\subsubsection{Extra simulation options}

Additional configuration options can be specified on the command line
with the \ttt{-\--}\textit{option}\ttt{=}\textit{value} syntax. (Note
that there is no space on either side of the equal sign.)

An example:

\begin{verbatim}
$ ./fifo --sim-time-limit=1000s
\end{verbatim}

Options given on the command line are stronger than the similar settings
in the ini files.
...
\begin{itemize}
  ...
  \item in ini files, with the \ttt{ned-path} configuration option
\end{itemize}


NED Path resoltion rules are as follows:
\begin{itemize}
  \item \opp checks for NED Path specified on the command line with the \ttt{-n} option
  \item if not found on the command line, checks for the NEDPATH environment variable
  \item the ned-path= entry from the INI file gets appended to the result of the above steps
  \item if the result is still empty it falls back to "." (current directory)
\end{itemize}
...
however you can override this with the \ttt{user-interface=Cmdenv} in your ini file or by
specifying -u Cmdenv on the command line. If both the config entry and the command line option 
is present the command line option takes precedence.


\subsubsection{Selecting a configuration or run}

Configurations can be selected with the \ttt{-c ConfigName} command line option. If you 
do not specify the configuration and you are running under:
...
You may get information about the configuration with the \ttt{-x ConfigName} option.
Adding the \ttt{-g} or \ttt{-G} option to the command line adds details about the
possible run numbers and iteration variables.

A specific run number can be selected by the \ttt{-r runnumber} option. It is also possible
specify more than one runs. Constructs like 1,2,5,7..9 or * can be used after the \ttt{-r}
option.

\begin{warning}
  It is recommended to execute only a single run this way. Specifying more than one runnumber 
  would run these simulations in the same process. This method is more prone the C++ programming 
  errors. A failure in a single run would stop and kill all the runs following it. If you want 
  to execute more than one run we recommend batch execution. Use the \fprog{opp\_runall} for 
  this purpose.
\end{warning}
...
the need to explicitly link against that library. This approach hase several advantages. 
...
\opp will attempt to load it using the \fname{dlopen()} or \fname{LoadLibrary()} functions and 
automatically registers all simple modules in the library.

The prefix and suffix from the library name can be omitted (the extension (\ttt{.dll, .si, .dylib} and also
...
It is also possible to specify the libraries to be loaded in the ini file with the \ttt{load-libs=} config entry.
The values from the command line and the config file will be merged. 

\begin{note}
  Of course runtime loading is not needed if your executable or shared lib was already linked against
  the library in question. In this case the plaform's dynamic loader will automatically load the library for you.
\end{note}

\begin{note}
  You must ensure that the library can be accessed by \opp. You have to specify the path with your library name 
  (pre and postfixes of the library filename still can be omitted) or adjust your shared 
  library path according to your OS. (On Windows set the PATH, on Unix set LD\_LIBRARY\_PATH and on Mac OS X
...
There are several command line options and configuration entries which control how a 
simulation is run.
...
TODO put somewhere: time limits, fingerprinting, and other options!, debug on error etc.
cmdenv-express-mode
cmdenv-interactive
...
Version: 4.0rc1, edition: Academic Public License -- NOT FOR COMMERCIAL USE
...
Total jobs processed: 1801720
Avg queueing time:    0.0102645
Max queueing time:    0.0410768
Standard deviation:   0.00135705
...
customized via \fname{omnetpp.ini} entries. The output file \fname{results/Fifo1-0.vec}
...
\begin{longtable}{lp{12cm}}
  ...
  \ttt{-f} \ttt{<}\textit{fileName\ttt{>}}
  &
\\

  \ttt{-l} \ttt{<}\textit{fileName\ttt{>}}
  &
  Load a shared object\index{shared objects} (\ttt{.so} file on Unix).
  Multiple \ttt{-l} switches are accepted. Your \ttt{.so} files may contain module
  code etc. By dynamically loading all simple
  module code and compiled network description (\ttt{\_n.o} files
  ...
  created with \ttt{gcc -shared...})\\

  \ttt{-r} \ttt{<}\textit{runs\ttt{>}}
  &
  It specifies which runs should be executed (e.g. \ttt{-r 2,4,6-8}).
  This option overrides the \ttt{runs-to-execute=} option
  in the \ttt{[Cmdenv]} section of the ini file\index{ini file}
  (see later).\\
\end{longtable}
...
Cmdenv can be executed in two modes, selected by the \ttt{cmdenv-express-mode}
ini file entry:
...
\fpar{cmdenv-performance-display} affects express mode only: it controls
...
\begin{longtable}{lp{12cm}}
  ...
  The default is \ttt{omnetpp.ini}\index{omnetpp.ini}.
...
\subsection{Past user interfaces}
...
TODO use \ttt{-h config} or \ttt{-h configdetails}
...
TODO express mode is on?

TODO eventlog recording is on?

TODO vector file recording is on?

TODO Tkenv: disable animation features, close inspectors, hide the timeline, hide object tree, etc.
