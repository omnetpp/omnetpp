...
By default modules are represented by simple icons, but this can
be overridden with the \ttt{b} or \ttt{i} tag. The \ttt{b} tag
is used to create simple geometric shapes:
\begin{verbatim}
...
Using images for the modules is also possible with the \ttt{i} tag.
...
The Server simple module 
also contains a \cclass{cQueue} object, which was named by the \fname{queue.setName("procname")}
...
that can be seen by hovering over them module with the mouse. 
TODO tooltip
TODO text
...
\begin{verbatim}
@display("t=This is the main server;tt=Additional tooltip information"); 
\end{verbatim}
...
\subsection{Specifying message display strings}
...
\subsection{Message display string tags}
