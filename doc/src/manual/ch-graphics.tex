...
One can obtain a pointer to the root figure of the canvas with the
\ffunc{getRootFigure()} member function, but that is usually unnecessary,
because the \cclass{cCanvas}, like \cclass{cFigure}, has methods for
accessing and manipulating its child figures directly.

One can add a child figure using \ffunc{addFigure()}. It has two flavours: one
...
\ffunc{getNumFigures()}). One can obtain the index of a child figure using
\ffunc{findFigure()}.
