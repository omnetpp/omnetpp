...
no explicit positions assigned (first two args of the "p=" tag
are missing) will be placed by the algorithm. Modules with fixed
coordinates will not be moved.

Predefined row, matrix, ring or other arrangements (defined
via the 3rd and further args of the "p=" tag) will be moved as one unit
...
If the full graph is too big after layouting, it is scaled back so that
it fits on the screen, UNLESS IT CONTAINS
