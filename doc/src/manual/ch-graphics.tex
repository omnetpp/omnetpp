...
order and the \cclass{cFigure} attribute called Z-index, with the latter
taking priority.
...
\ffunc{setZIndex()}/\ffunc{getZIndex()}, \ffunc{insertAbove()}/\ffunc{insertBelow()},
\ffunc{isAbove()}/\ffunc{isBelow()}, \ffunc{raiseAbove()}/\ffunc{lowerBelow()}.
