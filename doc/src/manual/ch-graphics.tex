...
of your main icon. Of course in most cases the \ttt{i2} tag is specified at runtime using
the \fname{setDisplayString()} method so the icon can be changed diynamically based on the
...
measured in pixles. You can use the parent module's \ttt{bgs=pix2unitratio,unit} tag, 
to set the scaling parameter and the physical unit for your module. 
You can specify the ratio between 1 pixel and 1 pysical unit with the \ttt{bgs} tag.
\end{note}
...
later at runtime using the \fname{setDisplayString()} method. The rest of the arguments
...
\begin{itemize}
  ...
  \item \ttt{column -- p=100,100,c,deltaY} (A comulm of modules with deltaX units between the modules)
  ...
  \item \ttt{exact (default) -- p=100,100,x,deltaX,deltaY} (Place each module at 100+deltaX;100+deltaY. The coordinates
         are usually set at runtime.)
\end{itemize}

TODO  screenshot of module vector
...
which was named by the \fname{queue.setName("procqueue")} method. 
...
\begin{note}
  ...
  \ttt{t} and \ttt{tt} tags are used at runtime with the \fname{setDisplayString()}
  method. 
\end{note}
...
the \fname{setDisplayString()} method at run time, but it would be also nice to
...
background (\ttt{bgi=icon,s}) and stretched to fill the module backgroud.
