...
dedicated method. It is called much more economically, that is, only
when needed.
...
\item If smooth animation is used, it is invoked continuously with a
      reasonably high frequency in \textit{Step} and \textit{Run} modes.
...
The stacking order a.ka. Z-order of figures is jointly determined by the
child order and the \cclass{cFigure} attribute called Z-index, with
the latter taking priority. Z-index is not used  directly, but an \textit{effective
Z-index} is computed instead, as the sum of the Z-index values of the figure
and all its ancestors up to the root figure.

Figures with a larger effective Z-index will be displayed above figures with
smaller effective Z-indices. Among figures whose effective Z-indices are equal,
the first child will be the bottom-most one, and the last child will be the
topmost one. For figures that are not siblings, the child order within the first
