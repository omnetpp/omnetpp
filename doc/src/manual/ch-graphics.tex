...
Inside a NED file, to include \ttt{,} \ttt{;} or \ttt{=} in a tag value, prefix it with two backslashes:
\ttt{\textbackslash\textbackslash,} \ttt{\textbackslash\textbackslash;} \ttt{\textbackslash\textbackslash=}.
To put a literal backslash in a tag value, write \ttt{{\textbackslash}{\textbackslash}{\textbackslash}{\textbackslash}} instead.
Tab or newline characters can be included in a tag value by writing them as \ttt{{\textbackslash}t} or \ttt{{\textbackslash}n}.

The reason it works this way is that display strings specified in NED files
are string literals, so backslash escape sequences are interpreted by the NED parser first.
This handles tabs, newlines, and literal backslashes and quotes.

Then, the string value is parsed by the display string parser, which splits the
string into tags and values. In this step, the remaining backslashes take away any special
meaning of commas, semicolons and equals signs.

Examples:

\begin{ned}
  @display("t=Hello\\, world!");
\end{ned}

\begin{ned}
  @display("t=This is a backslash character: '\\\\', this is a letter in quotes: \"x\".");
\end{ned}
...
Arbitrary NED expressions can also be embedded in display strings using the \ttt{\${expression}} notation.
These expressions are evaluated in the context of the display string's owner component, which means
that identifiers refer to the parameters of the component.

As opposed to the \ttt{\$foo} syntax, there is no fallback to parent component parameters.
To refer to the parent component's \ttt{foo} parameter, write this: \ttt{\${parent.foo}}.
...
To make passing parameters to functions easier, \ttt{,} does not separate tag values within
embedded expressions, so they don't have to be escaped within \ttt{\${}};


% escaping with "$$" or "$${}", "\\$" or "$\\{}" has no effect, "\\" disappears before that
