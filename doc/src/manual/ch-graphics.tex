...
The intent of the next sections are just to give you enough material to get started.
After that you are advised to refer to the material listed in the section
"OSG programming resources".
...
Alternatively, shapes can be built entirely from scratch using the
...
\iffalse
% this is the early draft version
...
- "using osgEarth" basically means that you load an .earth file via loadModel()

- everything that's true for OSG is also true for osgEarth, but it introduces additional APIs. they are in the osgEarth namespace

- here we give some hints on what's possible with osgEarth, and some tips/code fragments to access them
...
of the simulation, its appearance can be described in a .earth file. This is a
really versatile, XML based format.

It can be loaded using the \ffunc{osgDB::readNodeFile()} method, just like any
other regular model. The resulting \cclass{osg::Node} will have a descendant of
type \cclass{osgEarth::MapNode}, which can be easily found using the
\ffunc{osgEarth::MapNode::findMapNode()} function. This serves as the "view" part
of an underlying \cclass{osgEarth::Map} which is the "data model".
...
select whether the terrain should be projected onto a flat plane, or rendered as
a true geoid.
...
These can display different kinds of imagery, like real satellite photos, plain
rendering of the streets and other landmarks with their names, or any other
...
Elevation can also be streamed in a similarly simple fashion using the
...
Annotations can be of many kinds, for example simple geometry like circles,
ellipses, rectangles, lines, polygons (which can be extruded upwards to make
solids); text or labels, arbitrary 3D models, or images projected onto the
surface. Some style elements are
...
\subsubsection{Placing Objects on Terrain}
...
\subsubsection{Adding Annotations on Terrain}
