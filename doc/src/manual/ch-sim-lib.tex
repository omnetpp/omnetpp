...
The \fname{intrand()} function generates random integers in the range $[0, n-1]$.
\fname{dblrand()} generates a random double on $[0,1)$. Examples:
...
The underlying RNGs can be accessed via \cclass{cModule}'s \fname{rng()} method.
\begin{verbatim}
cRNG *rng1 = rng(1);
\end{verbatim}
