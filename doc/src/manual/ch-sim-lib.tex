...
simulation with an error message using the \fname{opp_error()} function.
\ttt{opp_error()}'s argument list works like \ttt{printf()}: the
...
objects owned can be done via \fname{forEachChild()} that loops through all
...
(e.g. from \fname{activity()}, \fname{handleMessage()}
...
to the module's object list.
  ...
  because that is the best guess a \fname{cMessage} constructor can do.}
...
to send it. When you get the error message (\ttt{"not owner of object"}),
