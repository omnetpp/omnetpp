...
\ttt{simtime\_t} which is a typedef to a class that stores simulation time
in a 64-bit integer using a fixed-point representation.
{\opp} provides utility functions to convert \ttt{simtime\_t}
...
or it is \ttt{nullptr}, \ffunc{simtimeToStr()} will place the result into a
static buffer which is overwritten with each call. An example:

\begin{cpp}
char buf[32];
EV << "t1=" << simtimeToStr(t1) << ", t2=" << simTimeToStr(t2,buf)) << endl;
\end{cpp}
