...
\cclass{cStatistic}'s \fname{random()} method, pointer to \cclass{cObject},
...
There is also a \fname{recordWithTimestamp()} method{\new}, to make it
...
stream output operator (\ttt{operator<<}) defined{\new}. By default,
...
Objects and pointers to objects can also be watched with WATCH\_OBJ() and WATCH\_PTR().
...
macros to watch STL containers:
WATCH\_VECTOR(),
WATCH\_PTRVECTOR(), WATCH\_LIST(), WATCH\_PTRLIST(), WATCH\_SET(),
WATCH\_PTRSET(), WATCH\_MAP(), WATCH\_PTRMAP().
...
subclassing from \cclass{cPolymorphic}, which does not impose any
...
\begin{itemize}
  ...
  \item{\textit{Duplication function,} \fname{cPolymorphic *dup() const}.
...
\begin{itemize}
  ...
  \item{\textit{Serialization}, \fname{netPack()} and \fname{netUnpack()} methods.
...
See the virtual functions of \cclass{cPolymorphic} and \cclass{cObject}
...
\textit{every \cclass{cObject}-based object has an owner all the time}.
...
The \fname{owner()} method, defined in \cclass{cObject}, returns the
owner of the object:
