...
\begin{itemize}
  ...
  \item{accessing other modules in the network (section \ref{ch:simple-modules:walking-module-hierarchy})}
...
\begin{itemize}
  \item{name attribute}
  \item{\fname{getClassName()} member and other member functions giving textual
...
For each class, the \fname{getClassName()} member function returns the class
name as a string:
...
and \fname{getName()} will return a pointer to a static \ttt{""}.
...
based on object names: \fname{getFullName()} and \fname{getFullPath()}.
For gates and modules which are part of gate or module vectors,
\fname{getFullName()} returns the name with the index in brackets.
That is, for a module \ttt{node[3]} in the submodule vector \ttt{node[10]}
\fname{getName()} returns \ttt{"node"}, and \fname{getFullName()} returns \ttt{"node[3]"}.
For other objects, \fname{getFullName()} is the same as \fname{getName()}.

\fname{getFullPath()} returns \fname{getFullName()}, prepended with the
parent or owner object's \fname{getFullPath()} and separated by a dot.
That is, if the \ttt{node[3]} module above is in the compound module
\ttt{"net.subnet1"}, its \fname{getFullPath()} method will return
...
\fname{getClassName()}, \fname{getFullName()} and \fname{getFullPath()}
are extensively used on the graphical runtime environment Tkenv,
and also appear in error messages.

\fname{getName()} and \fname{getFullName()} return \ttt{const char *} pointers,
and \fname{getFullPath()} returns \ttt{std::string}. This makes no difference
with \ttt{ev<<}, but when \fname{getFullPath()} is used as a \ttt{"\%s"} argument
to \fname{sprintf()} you have to write \ttt{getFullPath().c\_str()}.
...
\fname{dup()} delegates to the copy constructor, which in
...
\begin{note}
    Since the {\opp} 4.0 version, \fname{dup()} returns a pointer to
...
and they can be accessed via \cclass{cModule}'s \fname{getRNG()} method.
The argument to \fname{getRNG()} is a local RNG number which will undergo
RNG mapping.
...
The \fname{front()} and \fname{back()} functions return pointers to the objects
...
indices with an integer variable. The \fname{size()} member function
...
The \fname{getNumNodes()} member function (1st line) returns the number of
...
The \fname{getNodeFor()} member function returns a pointer to the graph
node for a given module. (If the module is not in the graph, it
returns \ttt{NULL}). \fname{getNodeFor()} uses binary search within the
\cclass{cTopology} object so it is fast enough.


\cclass{cTopology::Node}'s other member functions let you determine the
connections of this node: \fname{getNumInLinks()}, \fname{getNumOutLinks()} return
...
determine the modules and gates involved. The \fname{getRemoteNode()}
function returns the other end of the connection, and
\fname{getLocalGate()}, \fname{getRemoteGate()}, \fname{getLocalGateId()} and
\fname{getRemoteGateId()} return the gate pointers and ids of the gates
...
\fname{calculateUnweightedSingleShortestPathsTo()} overwrites the results of
the previous call.
...
The purpose of the \fname{getDistanceToTarget()} member function of a
node is self-explanatory. In the unweighted case, it returns the
number of hops. The \fname{getNumPaths()} member function returns the number
...
\fname{getNumPaths()} will always return 1 (or 0 if the target is not
...
\fname{getTargetNode()} function returns the target node of the last
...
algorithm. The \fname{isEnabled()} member function returns the state of
...
statistics out of the object: \fname{getCount()}, \fname{getMin()},
\fname{getMax()}, \fname{getMean()}, \fname{getStddev()}, \fname{getVariance()},
\fname{getSum()}, \fname{getSqrSum()} with the obvious meanings. An example
...
The \fname{isTransformed()} function returns \textit{true} when the cells have
...
are returned by the \fname{getUnderflowCell()} and \fname{getOverflowCell()}
member functions.
...
and the number of observations is each cell. \fname{getNumCells()} returns
the number of cells, \fname[getBasepoint()]{getBasepoint(int k)} returns the
\textit{k}th base point, \fname[getCellValue()]{getCellValue(int k)} returns the
number of observations in cell \textit{k}, and
\fname[getCellPDF()]{getCellPDF(int k)} returns the PDF value in the cell
(i.e. between \fname[getBasepoint()]{getBasepoint(k)} and
\fname[getBasepoint()]{getBasepoint(k+1)}).
The \fname[getCellInfo()]{getCellInfo(k)} method returns multiple data
...
The \fname[getPDF()]{getPDF(x)} and \fname[getCDF()]{getCDF(x)} member functions
...
\ref{sec:ch-config-sim:outvectors}).
...
Output scalars are recorded with the \fname{record()} method of
...
You can record whole statistics objects by calling their \fname{record()}
...
configuration entries (see \ref{sec:ch-config-sim:general-section}).
...
and \ttt{outputvectormanager-class=}; see \ref{sec:ch-config-sim:general-section}.
...
the \fname{getStackUsage()} call which tells you how much stack the module
...
\fname{getStackUsage()}also works by checking the existence of predefined
...
\begin{itemize}
  ...
  \item{\textit{Copy constructor}, which must have the following signature
        ...
        name (\fname{getName()}) of the other object it receives, then call the
        assignment operator (see below).}
  \item{\textit{Destructor}.}
  \item{\textit{Duplication function,} \fname{X *dup() const}.
...
The \fname{getOwner()} method, defined in \cclass{cOwnedObject}, returns the
owner of the object:
...
from that object's \fname{getOwner()} pointer.)
...
This is accomplished by the \fname{drop()} function, which transfers the
ownership to the object's default owner.
\fname{getDefaultOwner()} is a virtual method returning \ttt{cOwnedObject*}
...
invokes assignment, while \fname{dup()}
