...
\subsection{Simulation time conversion}
...
Its period length is $2^{31}-2$ with range $1...2^{31}-2$. The
formula is $x := (x * 7^{5}) mod (2^{31}-1)$, to be performed in
exactly 32-bit integer arithmetics, and one can verify the implementation
by asserting that if $x[0] = 1$ then $x[10000] = 1,043,618,065$.
...
an item size. If all three are 0 (\ttt{NULL}), no memory management is done,
...
pointers are \ttt{NULL}), \cclass{cPar} will use the delete operator to
...
For more information, see the reference and the code the NED compiler
generates for parameter expressions\index{parameter!expressions}.
...
object, or \ttt{NULL} if there's no redirection:
