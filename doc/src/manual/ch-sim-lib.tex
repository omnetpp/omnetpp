...
for any object by calling its \fname{setOwner()} member function.
...
In {\opp}, \cclass{cMessage} is a central class. Objects of \cclass{cMessage} and
...
You can add any number of parameters to a \cclass{cMessage}
object\index{message!adding parameters}.  Parameters are objects of
\cclass{cPar} type. You add a new parameter to the message with the
\fname{addPar()} member function:
...
object\index{message!accessing parameters} with the \fname{par()}
member function, and because \cclass{cPar} supports typecasting and
...
The \fname{addPar()} function also returns a reference to the added
\cclass{cPar} object, so you can set the value of the new parameter at
the same place:
...
You can use the \fname{hasPar()} function to see if the message has a 
...
accessed using an overloaded \fname{par()} function. Access by index
...
Message parameters are stored in an object of type \cclass{cArray}
which can store any object type not only
\cclass{cPars}\index{message!parameter storage}. The \fname{parList()}
member function lets you directly access the internal \cclass{cArray},
so by calling \cclass{cArray}'s member functions you can attach any
object to the message\index{message!attaching objects}. An example:
...
If you do not add parameters to the message and do not call the \fname{parList()} 
function, the internal \cclass{cArray} object will not be created. This 
...
It has been reported that using \cclass{cPar} message parameters might
...
probably better off subclassing\index{message!subclassing} either
\cclass{cMessage} or rather \cclass{cPacket}, and adding the required
parameters as ints, longs, bools, etc.  to the new message class.


Some time in the future {\opp} will directly support message 
...
then \cclass{cPar} parameters are the most convenient way to go.
...
The following functions exist in \cclass{cMessage}:
...
The \cclass{cPacket} constructor sets the message kind to MK\_PACKET. Both MK\_PACKET 
...
it uses the object ownership\index{ownership} management;
...
(\fname{submod()} is pointer to the current module the iterator is at.)
...
Does
\ttt{\fname{create()}+\fname{buildInside()}+\fname{callInitialize()}+\fname[scheduleStart()]{scheduleStart(now)}}.
...
\fname[uniform()]{double uniform(double lower\_limit, double upper\_limit);}\\
\fname[intuniform()]{double intuniform(double lower\_limit, double upper\_limit);}\\
\fname[exponential()]{double exponential(double mean);}\\
\fname[normal()]{double normal(double mean, double deviation);}\\
\fname[truncnormal()]{double truncnormal(double mean, double deviation);}\\
...
The counterparts of the above functions using generator \textit{k}:

\fname[genk\_uniform()]{double genk\_uniform(double k, double lower\_limit, double upper\_limit);}\\
\fname[genk\_intuniform()]{double genk\_intuniform(double k, double lower\_limit, double upper\_limit);}\\
\fname[genk\_exponential]{double genk\_exponential(double k, double mean);}\\
\fname[genk\_normal()]{double genk\_normal(double k, double mean, double deviation);}\\
\fname[genk\_truncnormal]{double genk\_truncnormal(double k, double mean, double deviation);}\\
...
be used with the \cclass{cPar} class and in NED files. You will find more 
information about this in the section describing \cclass{cPar}.
...
\cclass{cVarHistogram} class. TBD comments.


Now we do support the following two uses of \cclass{cVarHistogram}:
\begin{itemize}
  \item{add all the boundaries (manually) before collecting samples}
  \item{collect samples and \fname{transform()} makes the boundaries}
\end{itemize}

Transform types for \cclass{cVarHistogram}:
\begin{itemize}
  ...
  \item{\ttt{HIST\_TR\_AUTO\_EPC\_INT}: like the above, but uses a different 
    hack :-)}
\end{itemize}
...
\begin{itemize}
  ...
  \item{\fname{className()}: returns class name string}
  ...
  \item{\fname{inspectorFactoryName()}: used by Tkenv to create
    inspector windows\index{inspector windows} for objects of this type.}
\end{itemize}
...
The user can also specify a phase string\index{phase string} that is
...
is an effective way debugging\index{debugging}.
...
modules\index{module!color}, color/width of connection arrows etc.

\begin{sloppypar}
  The appearance of nodes and connections is determined by the display
  strings\index{display strings}. Display strings are initially taken
  from the NED description (stuff like: \ttt{display:
    ''p=100,10;i=pc''}). You can change the display string of a module
  or connection arrow at run-time by calling
  \fname{setDisplayString()}.  The display string of a connection
  arrow\index{arrow display string} is stored in its source gate.
...
\item{Turn off the display of screen messages when you run the
  ...
  \item{Use numeric index to get an object from a \cclass{cArray}, not the
...
\begin{itemize}
\item{message subclassing\index{message!subclassing}, and}
\item{using shared objects\index{shared objects}}
\end{itemize}
...
\cclass{cPar} objects and it frequently happens that a certain
...
This can be achieved with proper ownership\index{ownership}
...
parameters\index{shared parameters} alone and not to become their
...
the shared \cclass{cPar} objects and never do any harm to them.
