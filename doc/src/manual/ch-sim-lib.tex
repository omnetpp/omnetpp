...
The \cclass{cTopology} class provides powerful functionality for extracting and
...
The network topology can be extracted into a \cclass{cTopology} object with a
...
The \cclass{cTopology} class provides a corresponding extraction method for each
approach:
...
\ffunc{extractFromNetwork()}, which accepts a lambda function in modern C++.
...
A \cclass{cTopology} object uses the following classes to represent the graph:
\begin{itemize}
  \item \cclass{cTopology::Node}: Represents a module in the network
  \item \cclass{cTopology::Link}: Base class for edges in the graph
  \item \cclass{cTopology::LinkIn} and \cclass{cTopology::LinkOut}: Specialized
        views of a \cclass{Link} that provide convenient access to the "local" and
...
You can find the topology node that corresponds to a specific module using the
\ffunc{getNodeFor()} method:
...
The \cclass{cTopology::Link} class represents a connection in the graph, but
it is rarely used directly. Instead, \cclass{cTopology::LinkIn} and
\cclass{cTopology::LinkOut} provide convenient views of the same link from the
perspective of a specific node:

\begin{itemize}
  \item \cclass{LinkOut}: Represents an outgoing connection from the current node's perspective
  \item \cclass{LinkIn}: Represents an incoming connection from the current node's perspective
\end{itemize}

Both classes provide methods like \ffunc{getLocalNode()},
\ffunc{getRemoteNode()}, \ffunc{getLocalGate()}, and \ffunc{getRemoteGate()}
...
This design makes it easy to traverse the graph in either direction while
