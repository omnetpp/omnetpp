...
The network topology can be extracted into a \cclass{cTopology}
object with a single method call. There are several ways to specify
which modules should be included in the topology:

\begin{itemize}
  \item by NED types
  \item by module paths or module path patterns
  \item by a property's or parameter's presence and value
  \item by a user-supplied predicate function
\end{itemize}

The \cclass{cTopology} class has a member function for each of the above:
\ffunc{extractByNedTypeName()}, \ffunc{extractByModulePath()},
\ffunc{extractByModulePath()}, \ffunc{extractByProperty()},
\ffunc{extractByParameter()}, and \ffunc{extractFromNetwork()}.

The last method, the user-supplied predicate function, is the most general.
It expects the user to supply a function that accepts a module pointer,
and returns a boolean that indicates whether the module should be included in the topology.
An example that selects all top-level modules:

\begin{cpp}
cModule *root = getSimulation()->getSystemModule();
topo.extractFromNetwork([=](cModule *mod) {
    return mod->getParentModule() == root;
});
\end{cpp}
