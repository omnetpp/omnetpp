...
or \ttt{""} as its name string, it will be stored as \ttt{NULL}
...
should be set to \ttt{NULL} when the object to which it points is deleted.
...
There are two methods to define NED functions. The \fmac{Define\_NED\_Function()}
macro is the more flexible, preferred method of the two. \fmac{Define\_Function()}
is the older one, and it supports only certain cases. Both macros have
several variants.
...
there is no measurement unit, \ffunc{getUnit()} will return \ttt{NULL},
...
For unit conversion there are three functions. The
...
\cclass{cNEDValue} member accepts three arguments: a quantity as a
...
\subsection{Define\_Function()}

The \fmac{Define\_Function()} macro lets you register a C/C++
``mathematical'' function as a NED function. The registered C/C++ function
may take up to three \ttt{double} arguments, and must return a \ttt{double};
...
lists; and are restricted to three arguments at most. In exchange for these
restrictions, the C++ implementation of the functions is a lot simpler.

Accepted function signatures for \fmac{Define\_Function()}:
...
The simulation kernel uses \fmac{Define\_Function()} to expose commonly used
\ttt{<math.h>} functions in the NED language. Most \ttt{<math.h>} functions
(\ttt{sin()}, \ttt{cos()}, \ttt{fabs()}, \ttt{fmod()}, etc.)
...
\begin{cpp}
Define_Function(NAME,ARGCOUNT);
Define_Function2(NAME,FUNCTION,ARGCOUNT);
Define_Function3(NAME,ARGCOUNT,CATEGORY,DESCRIPTION);
Define_Function4(NAME,FUNCTION,ARGCOUNT,CATEGORY,DESCRIPTION);
\end{cpp}
...
\begin{cpp}
Define_Function3(sin, 1, "math", "Trigonometric function; see <math.h>");
Define_Function3(cos, 1, "math", "Trigonometric function; see <math.h>");
Define_Function3(pow, 2, "math", "Power-of function; see <math.h>");
\end{cpp}
...
The most significant features of \cclass{cOwnedObject} are
...
are better \textit{not} subclassed from \cclass{cOwnedObject}.
