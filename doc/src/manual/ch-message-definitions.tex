...
\begin{itemize}
  ...
  \item \textit{C++ blocks} are defined with the keyword \ttt{cplusplus}, and
...
To place the generated types into a namespace, a \ttt{namespace} directive
...
multiple nested \ttt{namespace} declarations as in C++:
...
There can be multiple \ttt{namespace} directives in a message file.

The effect of the \ttt{namespace} directive extends from the place of the
directive until the next \ttt{namespace} directive or the end of the message
file.
...
For boolean properties, the value may be \ttt{true} or \ttt{false}; it the value
is missing, \ttt{true} is assumed. Thus, \ttt{@customize} is equivalent to
\ttt{@customize(true)}.
...
\begin{itemize}
  ...
  \item \fkeyword{string}. Getters and setters use the \ttt{const char*} data
    type; \ttt{nullptr} is not allowed. Setters store a copy of the string, not
    just the pointer.
...
The getter and mutable getter return the stored pointer (or \ttt{nullptr} if
there is none).

The remover method releases the ownership of the stored object, sets the field
to \ttt{nullptr}, and returns the object.
...
type. For the built-in types (\ttt{int}, \ttt{double}, etc.) this information is
