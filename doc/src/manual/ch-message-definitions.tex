...
\begin{itemize}
  \item{\textit{Why doesn't it support \ttt{std::vector} and other STL classes?}
     ...
     to you -- so you can implement array fields using \ttt{std::vector}.
  ...
  \item{\textit{Why does it support C++ data types and not octets,
     ...
     be adopted.}
  \item{\textit{Why no inner classes?} Good question. As it does not
     conflict with the above principles, this feature might be added someday.}
\end{itemize}
...
messages. This might be sufficient for simple models, but if you
have more complex models, you will probably need to:

\begin{itemize}
  \item set up a hierarchy of message (packet) classes, that is,
    not only subclass from \cclass{cMessage}/\cclass{cPacket} but
    also from your own message classes;
...
\begin{itemize}
   \item variable-size arrays are not supported (no place for the array allocation code)
   \item ``generation gap'' or abstract fields (see later) cannot be used,
      because they build upon virtual functions.
\end{itemize}
