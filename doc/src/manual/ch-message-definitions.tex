...
The \fprop{@str} class property aims to simplify the implementation of an
\ffunc{str()} method in the generated class. When \fprop{@str} is present, an
\ttt{std::string str() const} method is generated with a single \ttt{return}
statement in its body. The statement returns the C++ expression specified as the
value of the \fprop{@str} property.
