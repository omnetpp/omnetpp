...
normally invoked automatically for \ttt{.msg} files during build.
...
\ttt{MyPacket} class as well as ``reflection'' code that allows inspection of
these data structures under graphical user interfaces like Qtenv. The
\ttt{MyPacket\_m.cc} file should be compiled and linked into the simulation;
...
Note that the methods are all declared \fkeyword{virtual} to allow overriding them.
...
\fkeyword{bool}. Several C/C++ and other data types can be used:
...
Using a \fprop{@enum} property, a field of the type \fkeyword{int} or any
...
latter makes it possible for Qtenv to display symbolic names.
...
The default array size is zero. This means that \ffunc{set...ArraySize(n)} needs
...
also be used as fields. For example, given a class named \ttt{IPAddress},
...
Example (section \ref{sec:msg-defs:using-cpp-types} will explain the
details):
...
\fkeyword{cplusplus} blocks that insert includes, typedefs, class/struct
...
A \fkeyword{cplusplus} block is also needed if the desired types are
...
A field is declared abstract by using \fkeyword{abstract } keyword:
...
\begin{enumerate}
  ...
  \item \ttt{getBar(int k)} cannot be implemented in a straightforward way
     ...
     and efficiency does not seem to be major problem because only Qtenv
  ...
  \item \ttt{setBar(int k, int x)} could not be implemented, but this
     ...
     Qtenv error dialog when you try to change the field value.
\end{enumerate}
...
message or packet in the simulation, Qtenv can uses the associated
...
class inspectable in Qtenv.) To do that, write a message definition for
