...
modules. Such series of connections from simple module to
simple module are called \textit{routes}.  Compound modules act like
`cardboard boxes' in the model, transparently relaying messages
between their inner realm and the outside world.


\subsection{Modeling of Packet Transmissions}

Connections can be assigned three parameters that facilitate
the modeling of communication networks, but can also be useful in
other models: \textit{propagation delay}, \textit{bit error rate}
...
Propagation delay is the amount of time by which the arrival of
the message is delayed when it travels through the channel.
...
Data rate is specified in bits/second, and used to calculate
transmission time of a packet.

When data rates are in use, the sending of the message in the model
corresponds to transmission of the first bit, and
arrival of the message corresponds to reception
of the last bit. This model is not always applicable;
for example protocols like Token Ring and FDDI do not wait
for the frame to arrive in its entirety; rather, they start repeating
its first bits soon after they arrive -- in other words,
frames ``flow through'' the stations, delayed by only a few bits.
If you want to model such networks, it is possible to change this default
behavior and deliver the message to the module when the first bit is
received.
...
the NED files or the configuration file omnetpp.ini.
...
files in a format that can be read into math packages like Matlab
...
Output scalar files can also be visualized with the {\opp} IDE.
It can draw bar charts, draw x-y plots (e.g., throughput vs. offered load),
