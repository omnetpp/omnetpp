...
limited. On the top level the user creates the system module as a network 
module which is a special compound module type without gates to the external world.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=3.772in, height=1.292in]{figures/over-modules}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=5.061in, height=1.121in]{figures/over-connections}
\caption{Connections}
...
When the program is started, first it reads all NED files\index{ned!files} 
...
Output scalar files can be visualized with the {\opp} IDE too.
