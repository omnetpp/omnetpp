...
The simple\index{module!simple} modules of a model contain the algorithms
as C++ functions.
The full flexibility and power of the programming language can
be used, supported by the {\opp} simulation class library.
Simulation programmer can choose between event-driven and process-style
description, and one can freely use object-oriented concepts
...
with C++ classes. They have been designed to work together efficiently,
...
\begin{itemize}
  \item{NED language topology description(s)\index{ned!files} (\texttt{.ned} files)
    ...
    NED files can be written with any text editor or using the
...
packages like Matlab or Octave, or imported into
spreadsheets like OpenOffice Calc, Gnumeric or MS Excel.
All these external programs have rich
...
Output scalar files can be visualized with the Scalars tool.
