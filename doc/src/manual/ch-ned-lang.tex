...
The following "allatorvosi lo" example shows the syntax for annotating
various NED elements:

\begin{Verbatim}
XXX syntax in all cases: submodule, connection etc....
\end{Verbatim}

Properties have a flexible data model. They may contain lists:

\begin{Verbatim}
@enum(hapci, morgo, kuka)
\end{Verbatim}
...
The above examples are special cases. According to the general data model,
properties contain \textit{key-valuelist} pairs, separated by semicolons.
Items in \textit{valuelist} are separated by commas. Wherever \textit{key}
is missing, values go on the valuelist of the \textit{default key}, the
empty string.
...
There are also some conventions. Boolean properties...
XXX how boolean properties are interpreted...
...
in the same context, then it names the exact same property object.
This might be a problem if you count on having multiple properties with
the same name.

if you want to add more than one property with the
same name, use an index.

@bubu(joe);
@bubu(jim);  // overrides the first one!!!
