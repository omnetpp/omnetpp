...
Parameters can get values from NED code, from the configuration (ini files,
see FIXME), or interactively from the user. It is also possible to give
them a default value (see \ttt{default(-1)} in the example), which gets
used if the parameter is not assigned at all in the configuration, or it is
explicitly declared to use the default (\ttt{=default} syntax).

XXX example: module and inifile:  =value;  =default; =ask; =expression
...
concatenation as well as numeric addition. Expressions can use several functions
(\ttt{fabs()}, \ttt{sin()}, \ttt{toUpper()}, etc.).

\begin{note}
    The list of NED functions can be read in Appendix \ref{cha:ned-functions}.
    The user can also extend NED with new functions; this feature is described
    in XXX.
\end{note}
...
by adding the \ttt{@unit} property. An example:
...
The \ttt{@unit} property of a parameter cannot be added or overridden
...
properties like the display string (\ttt{@display}). It is not possible to
...
The same syntax is used to add properties such as \ttt{@display} to the
connection.
...
Let us see more connection examples.
...
\ttt{@namespace} property in a \ffilename{package.ned} file affects all NED
...
\ffilename{package.ned} file in your \ttt{<root>} directory whose package
...
\ffilename{package.ned} has this property, other \ffilename{package.ned}'s cannot
change the package.
