...
We will express this model in terms of simple and compound modules, gates,
connections and so on, as introduced in the previous chapter.
...
Let us build the model bottom-up. First we define the components
that will make up a network node. We need an application; a module
that performs network layer functions including routing; and
a module that queues up packets before they are sent up to the
line.

The following NED code would be in files with \ttt{.ned} suffix.
Curly brace syntax...
...
Let us see the first declaration of the three.
It declares a simple module named \ttt{App}, which has four parameters
\ttt{address};  // local node address
        string destAddresses;  // destination addresses
        volatile double sendIaTime @unit(s) = default(exponential(1s));
                               // time between generating packets
        volatile int packetLength @unit(byte);  // length of one packet

\ttt{capacity} and \ttt{serviceTime}, and two gates \ttt{in} and \ttt{out}. The
lines starting with ``\ttt{//}'' are comments.

Note that the module name (\ttt{Queue}) begins with a capital letter, and
parameter and gate names (\ttt{capacity}, \ttt{serviceTime}, \ttt{in},
\ttt{out}) begin with lowercase. This is the recommended naming convention.
(Capitalization matters because the language is case sensitive).
...
the NED source more readable: in the \opp IDE they get displayed at various places (tooltips,
...
Now we assemble the above components into a compound module type that
...
Compound modules are modules that are composed of one or more submodules.
...
By convention, module type names begin with upper-case letters.
...
it has \ttt{gates} and \ttt{parameters} sections.
There are two additional sections, \ttt{submodules} and
\ttt{connections}.

All sections (\ttt{parameters}, \ttt{gates}, \ttt{submodules},
\ttt{connections}) are optional.

Then finally we define a network, which consists of a few nodes that
...
Naturally, only module that has no gates can be a network.
...
For example, a module called \ttt{TrafficGen} may have a parameter
...
\ttt{Backbone} channel inherits this C++ class from \ttt{ned.BasicChannel}.
...
\ttt{ned.BasicChannel} channel type. (In the latter name, \ttt{ned} is the
package name, and \ttt{BasicChannel} is the actual type name.)
\ttt{ned.BasicChannel} is a built-in NED type, and it declares four parameters:
...
\ttt{ned.BasicChannel} initializes these parameters to provide an ideal channel
