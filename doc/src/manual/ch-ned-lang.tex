...
NED files have the \ttt{.ned} file name suffix. The location of the files
...
the same as the module name (plus \ttt{.ned}).
...
Let us see a compound module from the \textit{Routing} example simulation:
...
Parameters affecting the internal structure should not be declared
\ttt{volatile}, as it may lead to unexpected results.
...
by specifying more than one indices in the \ttt{for} statement,
...
A \textit{network} is nothing else than a module type (simple or compound
module) which is a self-contained simulation model itself.

A module type becomes a network by adding the \ttt{@isNetwork(true)} property
to it. Examples:
...
Since a compound module with \ttt{@isNetwork(true)} occurs very frequently, there
is a shortcut for it: the \ttt{network} keyword. Thus, the above example can be
...
NED expressions have a C-like syntax, with
