...
simple module may send and receive messages via gates.

Gates allow one-to-one connections

gate vectors... bracket pair []
Elements of the vector are numbered from zero.

Gate vectors can also be created with
a specified size, or be set to a fixed size at the place
of instantiating the module.

The sizes of gate vectors may be given later, when the module is used as
a building block of a compound module type. Thus, every instance of
the module can have gate vectors of different sizes.

inout basically means that each gate is in fact a pair of gates,
an input and an output gate; they can also be addressed individually if needed,
as \ttt{port\$i} and \ttt{port\$o} (or for vector gates, as \ttt{port\$i[$k$]}
and \ttt{port\$o[$k$]})
