...
With channels, the default base type is \ttt{ned.BasicChannel}, so the
\ttt{extends\ldots} bit can be left out. Also, the \ttt{parameters:} keyword
is optional (everywhere, not only with channels), so our declaration can be
simplified to the following, completely equivalent one:
...
channel MyChannel extends ned.IdealChannel
{
   @class(MyChannel);
}

r1.g <--> {delay=10ms} <--> r2.g;
