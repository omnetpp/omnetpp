...
network Net6
{
    ...
    connections:
        ...
        node4.port++ <--> node6.port++;
        node1.port++ <--> node3.port++;
        node3.port++ <--> node5.port++;
        node5.port++ <--> node6.port++;
        node2.port++ <--> node5.port++;
        node3.port++ <--> node4.port++;
}
\end{Verbatim}

What ++ means etc

We'll cover it in the next section where \ttt{Node} comes from.
...
Node: it has gates, will be responsible for traffic generation and routing.
XXX add separate "user" nodes to the network?
...
Corresponding C++ class is \ttt{Node}; it needs to subclass from \cclass{cSimpleModule}
etc (link to next chapter!)


\section{Parameters}

Let us add parameters to our Node module type.
...
\section{Compound module}

compound module syntax (sections etc)

Replace Node with App, Router, L2Queue...
...
Replace node1, node2 etc with node[]:
