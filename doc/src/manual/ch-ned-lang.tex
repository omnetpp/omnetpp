...
\ttt{ns}   & nanoseconds (10$^-9$s) \\\hline
\ttt{us}   & microseconds (10$^-6$s) \\\hline
\ttt{ms}   & milliseconds (10$^-3$s) \\\hline
\ttt{s}    & seconds \\\hline
...
means that if compound module doesn't have such parameter,
further modules up in the module hierarchy will be searched for the parameter.
\ttt{ancestor} is considered evil because it violates the encapsulation
...
From the C++ code you'd access the XML element in the following way:
...
\cclass{cXMLElement} is documented in Chapter \ref{the-simulation-library}.

You can also XML parameters from omnetpp.ini:
...
\ttt{xmldoc()} with two arguments accepts an XPath-like expression
...
\begin{itemize}
  ...
  \item A path component can be an element tag name, "\ttt{*}", "\ttt{.}"
        or "\ttt{..}". Element tag names and "\ttt{*}" can have an optional predicate
...
These interpretations are consistent with the XPath specification.
  ...
  (finding elements in XML config files) the above support is completely
  enough.}

Examples:

% FIXME list becomes bold in HTML after a while

\begin{itemize}
  \item \ttt{/foo} -- the root element which must be called <foo>
  \item \ttt{/foo/bar} -- first <bar> child of the <foo> root element
  \item \ttt{//bar} -- first <bar> anywhere (depth-first search!)
  \item \ttt{/*/bar} -- first <bar> child of the root element which may have any tag name
  \item \ttt{/*/*/bar} -- first <bar> child two levels below the root element
  \item \ttt{/*/foo[0]} -- first <foo> child of the root element
  \item \ttt{/*/foo[1]} -- second <foo> child of the root element
  \item \ttt{/*/foo[@color='green']} -- first <foo> child which has attribute "color" with value "green"
  \item \ttt{//bar[1]} -- a <bar> element anywhere which is the second <bar> among its siblings
  \item \ttt{//*[@color='yellow']} -- any element anywhere which has attribute "color" with value "yellow"
  \item \ttt{//*[@color='yellow']/foo/bar} -- first <bar> child of first <foo> child of a "yellow-colored" element anywhere
\end{itemize}
