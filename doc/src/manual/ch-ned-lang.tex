...
algorithms. Currently, \ttt{RoutingTestNetwork} has \texttt{DistVecRouting}
...
say \ttt{routingAlg} to the \ttt{RoutingTestNetwork} compound module.
Then you can tell NED that types of the submodules inside \ttt{RoutingTestNetwork}
are not of any fixed module type, but contained in the \ttt{routingAlg}
...
whereas you have no \ttt{FooBarRouting} module implemented,
...
Namely, you have to name an existing module type (say \ttt{RoutingAlg})
and promise NED that all modules you're going you specify
in the \ttt{routingAlg} parameter will have (at least) the same
parameters and gates as the \ttt{RoutingAlg} module.
  \footnote{If you like, the above solution somewhat similar to polymorphism
  in object-oriented languages -- \ttt{RoutingAlg} is like a
  ``base class'', \texttt{DistVecRouting} and \texttt{AntNetRouting1}
  are like ``derived classes'', and the \ttt{routingAlg} parameter
...
The \texttt{RoutingAlg} module type does not need to be implemented in
...
\ttt{double}s are converted to \ttt{unsigned long} using the C/C++ builtin
conversion (type cast), the operation is performed, then the result
is converted back to \ttt{double}. Similarly, for the logical operators
\&\&, || and \#\#, the operands are converted to \ttt{bool} using the C/C++ builtin
conversion (type cast), the operation is performed, then the result
