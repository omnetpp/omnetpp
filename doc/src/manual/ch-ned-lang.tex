...
network Net6
{
    ...
    connections:
        ...
        node1.port++ <--> {datarate=100Mbps;} <--> node3.port++;
        node3.port++ <--> {datarate=100Mbps;} <--> node5.port++;
        node5.port++ <--> {datarate=100Mbps;} <--> node6.port++;
        node2.port++ <--> {datarate=100Mbps;} <--> node5.port++;
...
covered in more detail in sections XXX and XXX.

XXX explain datarate
...
Let us see the first simple module type declaration. \ttt{App} has some
parameters (omitted for now), and two gates named \ttt{out} and \ttt{in}
for sending and receiving application packets.
...
of the application (routing algorithm, queue) itself: that part is
expressed in C++. By default, {\opp} looks for C++ classes of the same name
as the NED type (so here, \ttt{App}, \ttt{Routing} and {Queue}), although
this gets slightly more complicated if C++ namespaces or NED packages are
involved, and the user can also explicitly specify which C++ class to use.

XXX describe @class and @namespace here?
...
compound module syntax (sections etc)
...
we'll explain submodule details and conns in the next section


. . . . . . . . . . . . . . . . . . . .

% XXX no comes everything in details!
...
\section{Properties (metadata annoations)}
...
It is also possible to give them a default value (see \ttt{default(-1)}
...
Numeric volatile parameters can be used as a source of random numbers
of various distributions. For example, setting a parameter to
\ttt{truncnormal(2,0.8)} would return a new random number
