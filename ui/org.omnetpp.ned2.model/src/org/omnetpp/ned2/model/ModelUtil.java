...
public class ModelUtil {
	...
	public static String generateNedSource(org.omnetpp.ned2.model.swig.NEDElement treeRoot) {
		NEDGenerator ng = new NEDGenerator();
		return ng.generate(treeRoot, "");
	}

	public static org.omnetpp.ned2.model.swig.NEDElement parseNedSource(String source) {
		NEDParser np = new NEDParser();
		np.setParseExpressions(false);
		np.parseText(source);
		org.omnetpp.ned2.model.swig.NEDElement treeRoot = np.getTree();
		// XXX just for debugging
		System.out.println(ModelUtil.printSwigElementTree(treeRoot, ""));
		return treeRoot;
	}

	public static org.omnetpp.ned2.model.swig.NEDElement loadNedSource(String fname) {
		NEDParser np = new NEDParser();
		np.setParseExpressions(false);
		np.parseFile(fname);
		org.omnetpp.ned2.model.swig.NEDElement treeRoot = np.getTree();
	...
	public static String printSwigElementTree(org.omnetpp.ned2.model.swig.NEDElement swigNode, String indent) {
	...
	public static EObject swig2pojo(NEDElement swigNode) {
		...
		// set the attributes
		for (int i = 0; i < swigNode.getNumAttributes(); ++i) {
			EStructuralFeature eFeature = Swig2EmfMapper.getEStructuralFeature(
					swigNode.getTagName(), swigNode.getAttributeName(i));
			...
			// convert the true/false string to booolean. all other attributes
		...
		for (NEDElement child = swigNode.getFirstChild(); child != null; child = child
			...
			if (emfChild == null)
				continue;
			EStructuralFeature eFeature = Swig2EmfMapper.getEStructuralFeature(
					swigNode.getTagName(), child.getTagName());
			System.out.println("  Assigning: " + swigNode.getTagName() + "."
					+ child.getTagName() + " -> " + eFeature);
			if (eFeature == null)
				continue;

			// if more than one child is possible (ie. the feature is an EList
			// -> add the child to the EList)
			...
			if (childContainer instanceof EList)
				...
				emfNode.eSet(eFeature, emfChild);
		}
