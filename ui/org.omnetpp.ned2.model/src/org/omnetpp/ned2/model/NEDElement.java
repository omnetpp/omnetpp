...
public abstract class NEDElement extends PlatformObject implements Iterable<NEDElement>
{
	 ...
	 * @param attr
	 */
	protected void fireAttributeChangedToAncestors(String attr, Object newValue, Object oldValue) {
		if (!isNotifyEnabled())
			return;

		NEDElement node = this;
		while (node != null) {
            node.fireAttributeChanged(this, attr, newValue, oldValue);
			node = node.getParent();
        }
	}
	...
	protected void fireChildRemovedToAncestors(NEDElement child) {
		...
		NEDElement node = this;
        while (node != null) {
            node.fireChildRemoved(this, child.getNextSibling(), child);
            node = node.getParent();
        }
	}
    ...
    // listener handling
    public void fireAttributeChanged(NEDElement source, String attr, Object newValue, Object oldValue) {
        if(listeners == null || !isNotifyEnabled())
            return;
        
        listeners.fireModelChanged(new NEDAttributeChangeEvent(source, attr, newValue, oldValue));
    }

    public void fireChildInserted(NEDElement source, NEDElement where, NEDElement child) {
        if(listeners == null || !isNotifyEnabled())
            return;
        listeners.fireModelChanged(
                new NEDStructuralChangeEvent(source, child, NEDStructuralChangeEvent.Type.INSERTION,
                                             where, null));
    }

    public void fireChildRemoved(NEDElement source, NEDElement where, NEDElement child) {
        if(listeners == null || !isNotifyEnabled())
            return;
        listeners.fireModelChanged(
                new NEDStructuralChangeEvent(source, child, NEDStructuralChangeEvent.Type.REMOVAL,
                                             null, where));
    }
