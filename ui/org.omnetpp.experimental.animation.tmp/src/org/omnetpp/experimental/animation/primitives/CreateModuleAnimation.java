...
public class CreateModuleAnimation extends AbstractInfiniteAnimation {
	...
	public void redo() {
		...
		if (parentModule == getSimulation().getRootModule()) { //FIXME
			...
			CompoundModuleFigure compoundModuleFigure = getCompoundModuleFigure(parentModule);
			compoundModuleFigure.getSubmoduleLayer().add(submoduleFigure);
			submoduleFigure.setDisplayString(compoundModuleFigure.getScale(), new DisplayString(""));
	...
	public void undo() {
		...
		if (parentModule != null) {
			...
			getCompoundModuleFigure(parentModule).getSubmoduleLayer().remove(submoduleFigure);
		}
