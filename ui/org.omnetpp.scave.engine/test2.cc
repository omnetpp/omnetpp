...
#include "engine/commonnodes.h"
#include "engine/dataflowmanager.h"
#include "engine/vectorfilereader.h"

class RandomSourceNode : public SingleSourceNode
{
    protected:
...
main()
{
    ...
    try
    {
        ...
        net.connect(&(src->out), &(winavg->in));
        net.connect(&(winavg->out), &(sink->in));
        */
        ...
        net.connect(src->addVector(7), &(pr1->in));
        net.connect(src->addVector(12), &(pr2->in));
