...
public class ModelCanvas extends EditorPart implements IInspectorContainer {
    public static final String EDITOR_ID = "org.omnetpp.simulation.inspectors.ModelCanvas";
    ...
    public void createPartControl(Composite parent) {
        ...
        canvas.getContents().setLayoutManager(new XYLayout());

        // recalculate canvas size when figures change or editor area gets resized
        canvas.getContents().addLayoutListener(new LayoutListener.Stub() {
            @Override
        ...
        FigureUtils.addTooltipSupport(canvas, canvas.getContents());

		// update inspectors when something happens in the simulation
		RuntimePlugin.getSimulationManager().addSimulationListener(simulationListener = new ISimulationListener() {
			//@Override
			public void changed() {
				refreshInspectors();
			}
    ...
    @Override
    public void dispose() {
    	RuntimePlugin.getSimulationManager().removeSimulationListener(simulationListener);
    ...
    protected void recalculateCanvasSize() {
        Dimension size = canvas.getContents().getPreferredSize();
    ...
    public void addInspectorPart(IInspectorPart inspectorPart) {
        int lastY = canvas.getContents().getPreferredSize().height;
    ...
    public void addInspectorPart(IInspectorPart inspectorPart, final int x, final int y) {
        ...
        canvas.getContents().add(moduleFigure);
        canvas.getContents().setConstraint(moduleFigure, new Rectangle(x, y, -1, -1));
    ...
    public void removeInspectorPart(IInspectorPart inspectorPart) {
    	...
    	canvas.getContents().remove(inspectorPart.getFigure());  //XXX maybe do it in the inspector??
