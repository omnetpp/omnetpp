...
public class GenericConfigPage extends FormPage {
	...
	public GenericConfigPage(Composite parent, String category, InifileEditor inifileEditor) {
		...
		if (category.equals(CAT_GENERAL)) {
			...
			addField(new TextFieldEditor(this, CFGID_RUNS_TO_EXECUTE, doc, "Runs to execute"));
			addField(new CheckboxFieldEditor(this, CFGID_EXPRESS_MODE, doc, "Run in Express mode"));
			addSpacer();
			Group group1 = createGroup(this, "Express mode");
			addField(new CheckboxFieldEditor(group1, CFGID_PERFORMANCE_DISPLAY, doc, "Display performance data"));
			addField(new TextFieldEditor(group1, CFGID_STATUS_FREQUENCY, doc, "Status frequency"));
			addSpacer();
			Group group2 = createGroup(this, "Normal (non-Express) mode");
			addField(new CheckboxFieldEditor(group2, CFGID_MODULE_MESSAGES, doc, "Print module messages"));
			addField(new CheckboxFieldEditor(group2, CFGID_EVENT_BANNERS, doc, "Print event banners"));
			addField(new CheckboxFieldEditor(group2, CFGID_EVENT_BANNER_DETAILS, doc, "Detailed event banners"));
			addField(new CheckboxFieldEditor(group2, CFGID_MESSAGE_TRACE, doc, "Message trace"));
			addSpacer();
			addField(new TextFieldEditor(this, CFGID_CMDENV_EXTRA_STACK_KB, doc, "Cmdenv extra stack (Kb)"));
			addField(new TextFieldEditor(this, CFGID_OUTPUT_FILE, doc, "Log file"));
			addField(new CheckboxFieldEditor(this, CFGID_AUTOFLUSH, doc, "Autoflush output files"));
		}
		else if (category.equals(CAT_TKENV)) {
			addField(new TextFieldEditor(this, CFGID_DEFAULT_RUN, doc, "Default run"));
			addSpacer();
			Group group4 = createGroup(this, "Paths");
			addField(new TextFieldEditor(group4, CFGID_IMAGE_PATH, doc, "Image path"));
			addField(new TextFieldEditor(group4, CFGID_PLUGIN_PATH, doc, "Plugin path"));
			addSpacer();
			...
			addField(new TextFieldEditor(group5, CFGID_SLOWEXEC_DELAY, doc, "Slow-exec delay"));
			addField(new TextFieldEditor(group5, CFGID_UPDATE_FREQ_FAST, doc, "Update frequency for Fast mode"));
			addField(new TextFieldEditor(group5, CFGID_UPDATE_FREQ_EXPRESS, doc, "Update frequency for Express mode"));
			addField(new CheckboxFieldEditor(group5, CFGID_EXPRESSMODE_AUTOUPDATE, doc, "Auto-update in Express mode"));
			addSpacer();
			Group group1 = createGroup(this, "Animation options");
			addField(new CheckboxFieldEditor(group1, CFGID_ANIMATION_ENABLED, doc, "Enable animation"));
			addField(new CheckboxFieldEditor(group1, CFGID_NEXT_EVENT_MARKERS, doc, "Show next event markers"));
			addField(new CheckboxFieldEditor(group1, CFGID_SENDDIRECT_ARROWS, doc, "Show SendDirect arrows"));
			addField(new CheckboxFieldEditor(group1, CFGID_ANIM_METHODCALLS, doc, "Animate method calls"));
			addField(new TextFieldEditor(group1, CFGID_METHODCALLS_DELAY, doc, "Delay for method call animation"));
			addField(new CheckboxFieldEditor(group1, CFGID_ANIMATION_MSGNAMES, doc, "Show message names in animation"));
			addField(new CheckboxFieldEditor(group1, CFGID_ANIMATION_MSGCLASSNAMES, doc, "Show message class names in animation"));
			addField(new CheckboxFieldEditor(group1, CFGID_ANIMATION_MSGCOLORS, doc, "Colorize messages by message kind"));
			addField(new CheckboxFieldEditor(this, CFGID_SHOW_BUBBLES, doc, "Show bubbles"));
			addField(new CheckboxFieldEditor(group1, CFGID_PENGUIN_MODE, doc, "Penguin mode"));
			addSpacer();
			Group group2 = createGroup(this, "Network layouting");
			addField(new CheckboxFieldEditor(group2, CFGID_SHOW_LAYOUTING, doc, "Show layouting process"));
			addField(new CheckboxFieldEditor(group2, CFGID_USE_NEW_LAYOUTER, doc, "Use new layouter"));
			addField(new TextFieldEditor(this, CFGID_ANIMATION_SPEED, doc, "Animation speed"));
			addSpacer();
			Group group3 = createGroup(this, "Logging");
			addField(new CheckboxFieldEditor(group3, CFGID_PRINT_BANNERS, doc, "Print banners"));
			addField(new CheckboxFieldEditor(group3, CFGID_USE_MAINWINDOW, doc, "Use main window"));
		}
