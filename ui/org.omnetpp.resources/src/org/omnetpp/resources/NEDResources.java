	...
	private void readNEDFile(IFile file) {
		...
		if (tree==null) {
			...
			try {
				...
				if (marker.exists()) {
					marker.setAttribute(IMarker.MESSAGE, "Cannot parse"); // XXX use actual parser error msg
					marker.setAttribute(IMarker.SEVERITY, IMarker.SEVERITY_ERROR);
					marker.setAttribute(IMarker.LINE_NUMBER, 1); // XXX use actual value
					marker.setAttribute(IMarker.LOCATION, "line X"); // XXX use actual value
				}
			} catch (CoreException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
