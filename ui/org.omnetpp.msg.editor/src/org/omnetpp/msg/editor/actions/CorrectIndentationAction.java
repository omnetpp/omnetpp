...
/**
 * Corrects the indentation level of lines in the selected region of the source.
 *
 * @author andras
 */
public class CorrectIndentationAction extends MsgTextEditorAction {
	public static final String ID = "CorrectIndentation";

	public CorrectIndentationAction(ITextEditor editor) {
		super(ID, editor);
	}
	...
	private static boolean containsSectionKeyword(String line) {
		line = removeCommentsAndStrings(line);
		return line.matches("(?s).*\\b("+StringUtils.join(Keywords.MSG_SECTION_KEYWORDS, "|")+")\\b.*");
	}
