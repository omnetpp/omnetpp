    ...
    private void createDemoNetwork() {
        {
            ...
            showMessage("Let's check the created NED file in text form too", 1, 100);
    ...
    private void createIniFile() {
        {
            ...
            showMessage("Let's run the simulation with 30 and then with 60 initial jobs. " +
            		"To do this we specify a 'running' parameter value using the ${jobs=30,60} syntax", 3);
            tree2.findTreeItemByContent("\\*\\*\\.source\\.numJobs").clickAndTypeOver(1, "${jobs=30,60}\n");
            sleep(2);
    
            // add queue parameters from dialog
            showMessage("Now specify the service time for all queues in the system", 2);
            addKeysButton.selectWithMouseClick();
            sleep(2);
    ...
    private void createLaunchConfigAndLaunch() {
        {
            ...
            showMessage("We will use the command line environment to run the simulation", 1);
            ...
            showMessage("In the INI file we have specified that the simulation should be run with " +
            		...
            		"This would result a total of 6 runs. We ant to run each of them so we specify * (all) as a run number ", 5);
