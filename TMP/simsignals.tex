...
\fname{hasListeners()} method can be used to check beforehand whether
...
The \fname{listenerRemoved()} method is also called from cComponent's destructor for all
...
\cclass{cObject}. For size limitations we won't discuss all classes in details --
...
XXX one example listener that reacts on, say, cPreModuleDeleteNotification
