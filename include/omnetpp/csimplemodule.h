...
class SIM_API cSimpleModule : public cModule //implies noncopyable
{
  ...
  private:
    enum {
        FL_USESACTIVITY        = 1 << 11, // uses activity() or handleMessage()
        FL_ISTERMINATED        = 1 << 12, // for both activity and handleMessage modules
        FL_STACKALREADYUNWOUND = 1 << 13, // only for activity modules
    };
